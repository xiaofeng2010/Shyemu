...
//		switch( GetByte(GAMEOBJECT_BYTES_1,GAMEOBJECT_BYTES_TYPEID) )
		...
		if( flags != 0x0352 && GetTypeId() == TYPEID_GAMEOBJECT && static_cast<GameObject*>(this)->GetInfo() && static_cast<GameObject*>(this)->GetInfo()->Type == GAMEOBJECT_TYPE_TRANSPORT && !(static_cast<GameObject*>(this)->GetOverrides() & GAMEOBJECT_OVERRIDE_PARENTROT) )
			flags = 0x0352;
	}
...
bool Object::SetPosition( float newX, float newY, float newZ, float newOrientation, bool allowPorting )
{
	...
	if( GetTypeId() == TYPEID_GAMEOBJECT && static_cast<GameObject*>(this)->GetInfo() && static_cast<GameObject*>(this)->GetInfo()->Type == GAMEOBJECT_TYPE_TRANSPORT )
		updateMap = true;
...
void Object::SendMessageToSetLocalized(WorldPacket *data, bool bToSelf,bool myteam_only, uint32 IdLocalized)
{
	...
	if(myteam_only)
	{
		...
		if(gminvis && data->GetOpcode()!=SMSG_MESSAGECHAT)
		{
			for(; itr != it_end; ++itr)
			{
				if((*itr) && (*itr)->GetSession() && (*itr)->GetSession()->IsLocalized(IdLocalized) && (*itr)->GetSession()->GetPermissionCount() > 0 && (*itr)->GetTeam()==myteam)
		...
		else
		{
			for(; itr != it_end; ++itr)
			{
				if((*itr) && (*itr)->GetSession() && (*itr)->GetSession()->IsLocalized(IdLocalized) && (*itr)->GetTeam()==myteam && !(*itr)->Social_IsIgnoring( GetLowGUID() ))
					(*itr)->GetSession()->SendPacket(data);
			}
		}
	}
	else
	{
		if(gminvis && data->GetOpcode()!=SMSG_MESSAGECHAT)
		{
			for(; itr != it_end; ++itr)
			{
				if((*itr) && (*itr)->GetSession() && (*itr)->GetSession()->IsLocalized(IdLocalized) && (*itr)->GetSession()->GetPermissionCount() > 0)
					(*itr)->GetSession()->SendPacket(data);
			}
		}
		else
		{
			for(; itr != it_end; ++itr)
			{
				if((*itr) && (*itr)->GetSession() && (*itr)->GetSession()->IsLocalized(IdLocalized) && !(m_objectTypeId == TYPEID_PLAYER && (*itr)->Social_IsIgnoring( GetLowGUID() )) )
					(*itr)->GetSession()->SendPacket(data);
			}
		}
	}
...
/*						//this commented code is not used because it was never tested and finished !
	...
	else /* ---------- NOT DEAD YET --------- */
	{
		...
		if(spellId && (pVictim->HasAura(30302) || pVictim->HasAura(30301) || pVictim->HasAura(30299)))
		{
			uint32 chance = RandomUInt(100);
			bool OK = false;
			if(pVictim->HasAura(30302))
			{
				if(chance <= 30)
					OK = true;
			}
			else if(pVictim->HasAura(30301))
			{
				if(chance <= 20)
					OK = true;
			}
			...
			if(OK == true)
			{
				...
				if(ent && ent->School)
				{
					...
					{
					case SCHOOL_HOLY:
						pVictim->CastSpell(pVictim, 54370, false);
						break;
					case SCHOOL_FIRE:
						pVictim->CastSpell(pVictim, 54371, false);
						break;
					case SCHOOL_NATURE:
						pVictim->CastSpell(pVictim, 54375, false);
						break;
					case SCHOOL_FROST:
						pVictim->CastSpell(pVictim, 54372, false);
						break;
					case SCHOOL_SHADOW:
						pVictim->CastSpell(pVictim, 54374, false);
						break;
					case SCHOOL_ARCANE:
						pVictim->CastSpell(pVictim, 54373, false);
						break;
					}
				}
			}
