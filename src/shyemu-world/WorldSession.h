...
public:
	...
	SHYEMU_INLINE void SendPacket(WorldPacket* packet)
	{
		if(_socket && _socket->IsConnected())
			_socket->SendPacket(packet);
	}

	SHYEMU_INLINE void SendPacket(StackBufferBase * packet)
	{
		if(_socket && _socket->IsConnected())
			_socket->SendPacket(packet);
	}

	SHYEMU_INLINE void OutPacket(uint16 opcode)
	{
		if(_socket && _socket->IsConnected())
	...
	SHYEMU_INLINE uint32 GetAccountId() const { return _accountId; }
	SHYEMU_INLINE Player* GetPlayer() { return _player; }
	...
	SHYEMU_INLINE char* GetPermissions() { return permissions; }
	SHYEMU_INLINE int GetPermissionCount() { return permissioncount; }
	SHYEMU_INLINE bool HasPermissions() { return (permissioncount > 0) ? true : false; }
	SHYEMU_INLINE bool HasGMPermissions()
	{
		if(!permissioncount)
			return false;
	...
	SHYEMU_INLINE void SetSocket(WorldSocket *sock)
	{
		_socket = sock;
	}
	SHYEMU_INLINE void SetPlayer(Player *plr) { _player = plr; }
	
	SHYEMU_INLINE void SetAccountData(uint32 index, char* data, bool initial,uint32 sz)
	{
		ASSERT(index < 8);
	...
	SHYEMU_INLINE AccountDataEntry* GetAccountData(uint32 index)
	{
		ASSERT(index < 8);
	...
	SHYEMU_INLINE void QueuePacket(WorldPacket* packet)
	{
	...
	SHYEMU_INLINE WorldSocket* GetSocket() { return _socket; }
	...
	SHYEMU_INLINE void SetInstance(uint32 Instance) { instanceId = Instance; }
	SHYEMU_INLINE uint32 GetLatency() { return _latency; }
	SHYEMU_INLINE string GetAccountName() { return _accountName; }
	SHYEMU_INLINE const char * GetAccountNameS() { return _accountName.c_str(); }
	const char * LocalizedWorldSrv(uint32 id);
	const char * LocalizedMapName(uint32 id);
	const char * LocalizedBroadCast(uint32 id);
	const char * LocalizedInstanceSrv(uint32 id);

	bool IsLocalized(uint32 id);

	SHYEMU_INLINE uint32 GetClientBuild() { return client_build; }
	SHYEMU_INLINE void SetClientBuild(uint32 build) { client_build = build; }
	bool bDeleted;
	SHYEMU_INLINE uint32 GetInstance() { return instanceId; }
	Mutex deleteMutex;
	...
	SHYEMU_INLINE bool IsLoggingOut() { return _loggingOut; }

	// Vehicles
	SHYEMU_INLINE void SetActiveMover(WoWGuid guid)
	{
		m_MoverWoWGuid = guid;
...
private:
	...
	uint8 _updatecount;
public:
	SHYEMU_INLINE MovementInfo* GetMovementInfo() { return &movement_info; }
