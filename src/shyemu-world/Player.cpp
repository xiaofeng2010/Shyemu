...
SHYEMU_INLINE uint32 GetSpellForLanguage(uint32 SkillID)
{
	switch(SkillID)
	{
...
void Player::_EventExploration()
{
	...
	// Restriction of the level required to go to Northrend 
	if(GetMapId() == 571 && getLevel() < 68)
	{
		// Horde
		if (getRace() == 2 || getRace() == 5 || getRace() == 6 || getRace() == 8 || getRace() == 10)
		{
			// Orgrimar
			EventTeleport(1, 1353.969971, -4642.560059, 53.543499);
		}
		else	// Alliance
		{
			// Stormwind
			EventTeleport(0, -8290.882813, 1402.138062, 4.742967);
		}
	}
	...
	// Restriction of archerus dominate only in their area
	if((GetMapId() == 609 && at->AreaId == 4298 && at->ZoneId == 0)	|| (GetMapId() == 609 && at->AreaId == 4636 && at->ZoneId == 4298)	|| (GetMapId() == 609 && at->AreaId == 4356 && at->ZoneId == 4298))
	{
		if(HasFinishedQuest(12657))
		{
			CastSpell(this, dbcSpell.LookupEntry(51721), true);
		}
		else
		{
			removeSpellByHashName( SPELL_HASH_DOMINION_OVER_ACHERUS );
		}
	}
	else 
	{
	...
	// Mise en sanctuaire de la zone shop
	if(GetMapId() == 1 && at->AreaId == 987 && at->ZoneId == 440)
	{
		at->category = AREAC_SANCTUARY;
		at->AreaFlags = AREA_SANCTUARY;
	}

	if(m_bg && !m_bg->m_started && (m_bg->GetType() == BATTLEGROUND_ARENA_2V2 || m_bg->GetType() == BATTLEGROUND_ARENA_3V3 || m_bg->GetType() == BATTLEGROUND_ARENA_5V5))
	{
		/* Add the green/gold team flag */
		Aura * aura = AuraPool.PooledNew();
		if (!aura)
			return;
		aura->Init(dbcSpell.LookupEntry((GetTeamInitial()) ? 35775-m_bgTeam : 32725-m_bgTeam), -1, this, this, true);
		AddAura(aura);

		CastSpell(this, 32728, true);
	}
...
void Player::_LoadActionBars(QueryResult * result)
{
	// Load info from DB
	if(result)
	{
		do 
		{
			...
			{
				sLog.outDebug("Out of range spec number [%d] for player with GUID [%d] in playeractionbars", 
					spec, fields[0].GetUInt32());
				continue;
			}

			uint32 button = fields[3].GetInt32();

			for(uint32 i = 0; i < PLAYER_ACTION_BUTTON_COUNT; ++i)
			{
				m_specs[spec].actionbars[button].Action = fields[3].GetInt16();
				m_specs[spec].actionbars[button].Type = fields[4].GetInt8();
				m_specs[spec].actionbars[button].Misc = fields[5].GetInt8();
			}
...
void Player::_SaveSpellsToDB(QueryBuffer * buf)
{
	...
	for(; spellItr != mSpells.end(); ++spellItr)
	{
		...
		if( !sp || sp->RequiredShapeShift == 21 )
			continue;
		if(!first)
			ss << ",";
		else
...
bool Player::LoadFromDB(uint32 guid)
{
	...
	// Spells, Glyphs, Talents, Action Bars
	...
	q->AddQuery("SELECT * FROM playeractionbars WHERE guid = %u", guid);
...
#define get_next_field fields[field_index++]
	...
	m_maxTalentPoints										= get_next_field.GetUInt32();
...
//	UpdateMaxSkills();
	...
	while(true)
	{
		...
		if(!end)break;
		*end=0;
		SpellEntry * spProto = dbcSpell.LookupEntryForced(atol(start));
		if(spProto)
...
#undef get_next_field
	...
	// load properties
	_LoadTalents(results[11].result);
	_LoadGlyphs(results[12].result);
	_LoadSpells(results[13].result);
	_LoadActionBars(results[14].result);
...
void Player::UpdateStats()
{
	...
	case DRUID:
	{
		...
		if(HasAurasWithNameHash(SPELL_HASH_LUNAR_GUIDANCE))
		{
			uint32 id = HasAurasWithNameHash(SPELL_HASH_LUNAR_GUIDANCE);
			RemoveAura(id);
...
void Player::SendDualTalentConfirm()
{
   ...
   {
       // Not enough money!
       sChatHandler.SystemMessage(m_session, "You must have at least %u pieces of cooper to use this feature", talentCost);
   }
   /*WorldPacket data();
   data << GetGUID();
...
uint32 Player::TakeRunes(uint8 type, uint32 count)
{
	...
	for(uint8 i=0; i<6 && count != found; ++i)
	{
		...
		{
			//Drain Sanglant
			if( HasAura(45529) && type == RUNE_DEATH && (count == 1 || count == 0))
				RemoveAura(45529);
...
void Player::UpdatePotionCooldown(Spell* spell)
{
	...
	ItemPrototype * xproto=ItemPrototypeStorage.LookupEntry(m_lastPotionId);
	if(xproto)
	{
		for(int idx = 0; idx < 5; idx++)
		{
			if(xproto->Spells[idx].Id && xproto->Spells[idx].Trigger == 0)
				if(SpellEntry *spellInfo = dbcSpell.LookupEntry( xproto->Spells[idx].Id ))
				{
					_Cooldown_Add( COOLDOWN_TYPE_CATEGORY, spellInfo->Category, getMSTime() + spellInfo->CategoryRecoveryTime, spellInfo->Id, m_lastPotionId );
...
void Player::UnapplyGlyph(uint32 slot)
{
	...
	RemoveAllAuras(glyph->SpellID, GetGUID());
}
...
void Player::LearnTalent(uint32 talent_id, uint32 requested_rank)
{
	...
	if(currentRank > requested_rank + m_talentActiveSpec)
