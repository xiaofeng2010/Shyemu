 ...
 * Copyright (C) 2009-2010 EternalCoreTeam <http://eternal-realms.fr/>
 *
...
enum DeathState
{
	...
	DEAD_FALLING //Need to script it
};
...
public:
	...
	SHYEMU_INLINE void Vanished()
	{
		ClearAttackers();
		ClearHealers();
	}

	SHYEMU_INLINE const uint64& GetPrimaryAttackTarget() { return m_primaryAttackTarget; }
	SHYEMU_INLINE void SetUnit(Unit * p) { m_Unit = p; }
...
/*	typedef struct { const char *name; int(*mfunc)(lua_State*,Unit*); } RegType;
	...
	SHYEMU_INLINE void SetDualWield(bool enabled)
	{
		m_dualWield = enabled;
	}
  ...
  //void StrikeWithAbility( Unit* pVictim, Spell* spell, uint32 addspelldmg, uint32 weapon_damage_type );
	...
	SHYEMU_INLINE void addStateFlag(uint32 f) { m_state |= f; };
	SHYEMU_INLINE bool hasStateFlag(uint32 f) { return (m_state & f ? true : false); }
	SHYEMU_INLINE void clearStateFlag(uint32 f) { m_state &= ~f; };

	/// Stats
	SHYEMU_INLINE uint32 getLevel() { return m_uint32Values[ UNIT_FIELD_LEVEL ]; };
	SHYEMU_INLINE uint8 getRace() { return GetByte(UNIT_FIELD_BYTES_0,0); }
	SHYEMU_INLINE uint8 getClass() { return GetByte(UNIT_FIELD_BYTES_0,1); }
	SHYEMU_INLINE void setRace(uint8 race) { SetByte(UNIT_FIELD_BYTES_0,0,race); }
	SHYEMU_INLINE void setClass(uint8 class_) { SetByte(UNIT_FIELD_BYTES_0,1, class_ ); }
	SHYEMU_INLINE uint32 getClassMask() { return 1 << (getClass() - 1); }
	SHYEMU_INLINE uint32 getRaceMask() { return 1 << (getRace() - 1); }
	SHYEMU_INLINE uint8 getGender() { return GetByte(UNIT_FIELD_BYTES_0,2); }
	SHYEMU_INLINE void setGender(uint8 gender) { SetByte(UNIT_FIELD_BYTES_0,2,gender); }
	SHYEMU_INLINE uint8 getStandState() { return ((uint8)m_uint32Values[UNIT_FIELD_BYTES_1]); }
...
//	void HandleProcSpellOnSpell(Unit* Victim,uint32 damage,bool critical);//nasty, some spells proc other spells
	...
	SHYEMU_INLINE void setHRegenTimer(uint32 time) {m_H_regenTimer = static_cast<uint16>( time ); }
	SHYEMU_INLINE void setPRegenTimer(uint32 time) {m_P_regenTimer = static_cast<uint16>( time ); }
	SHYEMU_INLINE void DelayPowerRegeneration(uint32 time) { m_P_regenTimer = static_cast<uint16>( time ); if (!m_interruptedRegenTime) m_interruptedRegenTime = 2000; }
	void DeMorph();
	...
	// Stealth  
	SHYEMU_INLINE int32 GetStealthLevel() { return m_stealthLevel; }
	SHYEMU_INLINE int32 GetStealthDetectBonus() { return m_stealthDetectBonus; }
	SHYEMU_INLINE void SetStealth(uint32 id) { m_stealth = id; }
	SHYEMU_INLINE bool IsStealth() { return (m_stealth!=0 ? true : false); }
	float detectRange;

	// Invisibility
	SHYEMU_INLINE void SetInvisibility(uint32 id) { m_invisibility = id; }
	SHYEMU_INLINE bool IsInvisible() { return (m_invisible!=0 ? true : false); }
	...
	/// Combat / Death Status
	SHYEMU_INLINE bool isAlive() { return m_deathState == ALIVE; };
	SHYEMU_INLINE bool IsDead() { return  m_deathState !=ALIVE; };
	...
	uint32 m_LastHealAmount;
	uint32 m_lastAbsorbForAbsorbIncanter;
...
//	std::map<uint32,ProcTriggerSpellOnSpellList> m_procSpellonSpell; //index is namehash
	...
	SHYEMU_INLINE void SetOnMeleeSpell( uint32 spell, uint8 ecn = 0 ) { m_meleespell = spell; m_meleespell_ecn = ecn;  }
	SHYEMU_INLINE uint32 GetOnMeleeSpell() { return m_meleespell; }
	SHYEMU_INLINE uint8 GetOnMeleeSpellEcn() { return m_meleespell_ecn; }
	...
	void WipeHateList();
	void WipeTargetList();
	SHYEMU_INLINE void setAItoUse(bool value){m_useAI = value;}
...
//	virtual Group *GetGroup();
	...
	SHYEMU_INLINE float GetHitFromMeleeSpell() { return m_hitfrommeleespell; }
	float m_hitfrommeleespell;

	// DK:Affect
	SHYEMU_INLINE uint32 IsPacified() { return m_pacified; }
	SHYEMU_INLINE uint32 IsStunned() { return m_stunned; }
	SHYEMU_INLINE uint32 IsFeared() { return m_fearmodifiers; }
	SHYEMU_INLINE uint32 GetResistChanceMod() { return m_resistChance; }
	SHYEMU_INLINE void SetResistChanceMod(uint32 amount) { m_resistChance=amount; }
	
	SHYEMU_INLINE uint16 HasNoInterrupt() { return m_noInterrupt; }
	...
	void StopChannel();
	void DealDamageWithPenance(uint32 spellId, uint32 amount);
	...
	void EmoteExpire();
	SHYEMU_INLINE void setEmoteState(uint8 emote) { m_emoteState = emote; };
	SHYEMU_INLINE uint32 GetOldEmote() { return m_oldEmote; }
	...
	SHYEMU_INLINE StandState GetStandState()
	{
		uint32 bytes1 = GetUInt32Value (UNIT_FIELD_BYTES_1);
		return StandState (uint8 (bytes1));
	}

	SHYEMU_INLINE void SetFaction(uint32 factionId)
	{
		SetUInt32Value(UNIT_FIELD_FACTIONTEMPLATE, factionId );
		_setFaction();
	}

	void SendChatMessageLocalized(uint8 type, uint32 lang, uint32 IdLocalized);
	...
	SHYEMU_INLINE int GetHealthPct()
	{
    ...
    SHYEMU_INLINE void SetHealthPct(uint32 val) { if (val>0) SetUInt32Value(UNIT_FIELD_HEALTH,float2int32(val*0.01f*GetUInt32Value(UNIT_FIELD_MAXHEALTH))); };

	SHYEMU_INLINE int GetManaPct()
	{
	...
	SHYEMU_INLINE Spell * GetCurrentSpell(){return m_currentSpell;}
	SHYEMU_INLINE void SetCurrentSpell(Spell* cSpell) { m_currentSpell = cSpell; }

	uint32 m_CombatUpdateTimer;

	SHYEMU_INLINE void setcanperry(bool newstatus){can_parry=newstatus;}
	...
	SHYEMU_INLINE uint32 GetPowerType(){ return (GetUInt32Value(UNIT_FIELD_BYTES_0)>> 24);}
	void UpdatePowerAmm();
	...
	SHYEMU_INLINE uint32 GetCharmTempVal() { return m_charmtemp; }
	SHYEMU_INLINE void SetCharmTempVal(uint32 val) { m_charmtemp = val; }

	SHYEMU_INLINE void DisableAI() { m_useAI = false; }
	SHYEMU_INLINE void EnableAI() { m_useAI = true; }

	SHYEMU_INLINE void SetPowerType( uint8 type )
	{
		SetByte( UNIT_FIELD_BYTES_0, 3, type );
	}

	SHYEMU_INLINE bool IsSpiritHealer()
	{
		switch(GetEntry())
		{
...
//	bool m_spellsbusy;
	...
	SHYEMU_INLINE float GetBlockFromSpell() { return m_blockfromspell; }
	SHYEMU_INLINE float GetParryFromSpell() { return m_parryfromspell; }
	SHYEMU_INLINE float GetDodgeFromSpell() { return m_dodgefromspell; }
