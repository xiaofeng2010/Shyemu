...
#define BANNER "shyemu %s r%u/%s-%s-%s :: World Server"
...
bool Master::Run(int argc, char ** argv)
{
	...
	struct shyemu_option longopts[] =
	{
		{ "checkconf",			shyemu_no_argument,				&do_check_conf,			1		},
		{ "screenloglevel",		shyemu_required_argument,		&screen_log_level,		1		},
		{ "fileloglevel",		shyemu_required_argument,		&file_log_level,		1		},
		{ "version",			shyemu_no_argument,				&do_version,			1		},
		{ "conf",				shyemu_required_argument,		NULL,					'c'		},
		{ "realmconf",			shyemu_required_argument,		NULL,					'r'		},
		{ "databasecleanup",	shyemu_no_argument,				&do_database_clean,		1		},
		{ "cheatercheck",		shyemu_no_argument,				&do_cheater_check,		1		},
		{ 0, 0, 0, 0 }
	};

	char c;
	while ((c = static_cast<char>( shyemu_getopt_long_only(argc, argv, ":f:", longopts, NULL) ) ) != -1)
	{
		switch (c)
		{
		case 'c':
			config_file = new char[strlen(shyemu_optarg)];
			strcpy(config_file, shyemu_optarg);
			break;

		case 'r':
			realm_config_file = new char[strlen(shyemu_optarg)];
			strcpy(realm_config_file, shyemu_optarg);
			break;
...
#endif
	Log.Color(TBLUE);
	printf("\nCopyright (C) 2008-2009 shyemu. http://www.shyemu.org/\n");
	...
	printf("\n");
	printf(" Website: http://www.shyemu.org	     			\n");
	printf(" Forums:  http://www.shyemu.org/forums/   \n");
	printf(" Credits: http://www.shyemu.org/credits   \n");
	printf(" SVN:     http://shyemu.info/svn/         \n");
	printf(" Have fun!                                \n");
	Log.Line();
#ifdef REPACK
	Log.Color(TRED);
	printf("Warning: Using repacks is potentially dangerous. You should always compile\n");
	printf("from the source yourself at www.shyemu.org.\n");
	printf("By using this repack, you agree to not visit the shyemu website and ask\nfor support.\n");
	...
	if(geteuid() == 0 || getegid() == 0)
		Log.LargeErrorMessage( LARGERRORMESSAGE_WARNING, "You are running shyemu as root.", "This is not needed, and may be a possible security risk.", "It is advised to hit CTRL+C now and", "start as a non-privileged user.", NULL);
#endif
	...
	if (Config.MainConfig.GetIntDefault( "LogLevel", "DisableCrashdumpReport", 0) == 0)
	{
		...
		snprintf(cmd, 1024, "./shyemu-crashreport -r %d -d \"%s\"", BUILD_REVISION, banner);
		system(cmd);
	}
	unlink("shyemu.uptime");
#endif
