...
public:
	...
	SHYEMU_INLINE uint32 GetBlockCount() const {return mBlocks;}
	
	SHYEMU_INLINE uint32 GetLength() const { return (mBlocks *sizeof(uint32)); }
	SHYEMU_INLINE uint32 GetCount() const { return mCount; }
	SHYEMU_INLINE const uint8* GetMask() const { return (uint8*)mUpdateMask; }
