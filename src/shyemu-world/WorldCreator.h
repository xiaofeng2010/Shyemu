...
public:
	...
	SHYEMU_INLINE Map* GetMap(uint32 mapid)
	{
		if(mapid>NUM_MAPS)
			return NULL;
		else
	...
	// can a player join?
    SHYEMU_INLINE bool PlayerOwnsInstance(Instance * pInstance, Player * pPlayer)
	{
		// Expired?
	...
	// has an instance expired?
	SHYEMU_INLINE bool HasInstanceExpired(Instance * pInstance)
	{
		// expired?
		if( pInstance->m_expiration && (UNIXTIME+20) >= pInstance->m_expiration)
			return true;
