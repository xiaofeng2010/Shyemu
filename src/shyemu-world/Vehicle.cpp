...
void Vehicle::SendHeartbeatPacket(Player *player)
{
	...
	data << GetNewGUID();
	/*data << float (player->GetSession()->movement_info.transX);
    data << float (player->GetSession()->movement_info.transY);
    data << float (player->GetSession()->movement_info.transZ);
	data << float (player->GetSession()->movement_info.orientation);
	data << uint32(player->GetSession()->movement_info.time);
	data << uint32(player->GetSession()->movement_info.se);
	data << uint32(player->GetSession()->movement_info.FallTime);
    data << uint8 ();*/
