...
enum procFlags
{
	...
	//PROC_ON_OFFHAND 					= 0x100000000,	// signed value is out of range for enum constant - you need custom procflag for this
};
#else
...
SHYEMU_INLINE bool CanAgroHash(uint32 spellhashname)
{
...
/************************************************************************/
SHYEMU_INLINE bool IsDamagingSpell(SpellEntry *sp)
{
    switch (sp->Effect[0])
    {
...
SHYEMU_INLINE uint32 IsHealingSpell(SpellEntry *sp)
{
    switch( sp->Effect[0] )
    {
        case SPELL_EFFECT_HEALTH_LEECH:
...
SHYEMU_INLINE bool IsInrange(LocationVector & location, Object * o, float square_r)
{
...
SHYEMU_INLINE bool IsInrange(float x1,float y1, float z1,float x2,float y2, float z2,float square_r)
{
    float t;
    float r;
    t=x1-x2;
...
SHYEMU_INLINE bool IsInrange(Object * o1,Object * o2,float square_r)
{
    return IsInrange(o1->GetPositionX(),o1->GetPositionY(),o1->GetPositionZ(),
        o2->GetPositionX(),o2->GetPositionY(),o2->GetPositionZ(),square_r);
}

SHYEMU_INLINE bool TargetTypeCheck(Object *obj,uint32 ReqCreatureTypeMask)
{
...
SHYEMU_INLINE bool IsTargetingStealthed(SpellEntry *sp)
{
	if(
		HasTargetType(sp,EFF_TARGET_INVISIBLE_OR_HIDDEN_ENEMIES_AT_LOCATION_RADIUS) ||
		HasTargetType(sp,EFF_TARGET_ALL_ENEMIES_AROUND_CASTER) ||
...
public:
    ...
    uint8 CanCast(bool);
	SHYEMU_INLINE bool hasAttribute(uint32 attribute)
	{
		return ((GetProto()->Attributes & attribute) > 0);
	}
	SHYEMU_INLINE bool hasAttributeEx(uint32 attribute)
	{
		return ((GetProto()->AttributesEx & attribute) > 0);
	}
	SHYEMU_INLINE bool hasAttributeExB(uint32 attribute)
	{
		return ((GetProto()->AttributesExB & attribute) > 0);
	}
	SHYEMU_INLINE bool hasAttributeExC(uint32 attribute)
	{
		return ((GetProto()->AttributesExC & attribute) > 0);
	}
	SHYEMU_INLINE bool hasAttributeExD(uint32 attribute)
	{
    ...
    SHYEMU_INLINE uint32 getState() { return m_spellState; }
    SHYEMU_INLINE void SetUnitTarget(Unit *punit){unitTarget=punit;}
	...
	void SpellEffectConvertInDeathRunes(uint32 i);
	...
	SHYEMU_INLINE uint32 GetType() { return ( GetProto()->Spell_Dmg_Type == SPELL_DMG_TYPE_NONE ? SPELL_DMG_TYPE_MAGIC : GetProto()->Spell_Dmg_Type ); }

	TargetsList UniqueTargets;
    SpellTargetsList    ModeratedTargets;

    SHYEMU_INLINE Item* GetItemTarget() { return itemTarget; }
    SHYEMU_INLINE Unit* GetUnitTarget() { return unitTarget; }
    SHYEMU_INLINE Player* GetPlayerTarget() { return playerTarget; }
    SHYEMU_INLINE GameObject* GetGameObjectTarget() { return gameObjTarget; }
	...
	SHYEMU_INLINE SpellEntry* GetProto() { return (m_spellInfo_override == NULL) ?  m_spellInfo : m_spellInfo_override; }
    ...
    SHYEMU_INLINE float GetRadius(uint32 i)
    {
    ...
    SHYEMU_INLINE static uint32 GetBaseThreat(uint32 dmg)
    {
...
/*        switch(GetProto()->NameHash)
	...
	SHYEMU_INLINE static uint32 GetMechanic(SpellEntry * sp)
	{
		if(sp->MechanicsType)
	...
	SHYEMU_INLINE void safe_cancel()
	{
		m_cancelled = true;
	}
    ...
    /// Spell state's
    /// Spell failed
    SHYEMU_INLINE bool GetSpellFailed(){return m_Spell_Failed;}
    SHYEMU_INLINE void SetSpellFailed(bool failed = true){m_Spell_Failed = failed;}

    SHYEMU_INLINE bool IsReflected() {return m_IsReflected;}
    SHYEMU_INLINE void SetReflected(bool reflected = true) {m_IsReflected = reflected;}

    /// Spell possibility's
    SHYEMU_INLINE bool GetCanReflect() {return m_CanRelect;}
    SHYEMU_INLINE void SetCanReflect(bool reflect = true) {m_CanRelect = reflect;}
...
protected:
	...
	SHYEMU_INLINE bool HasTarget(const uint64& guid, TargetsList* tmpMap)
	{
