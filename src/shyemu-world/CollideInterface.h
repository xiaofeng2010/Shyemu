...
public:
	...
	SHYEMU_INLINE bool CheckLOS(uint32 mapId, float x1, float y1, float z1, float x2, float y2, float z2)
	{
		return CollisionMgr->isInLineOfSight( mapId, x1, y1, z1, x2, y2, z2 );
	}

	SHYEMU_INLINE bool GetFirstPoint(uint32 mapId, float x1, float y1, float z1, float x2, float y2, float z2, float & outx, float & outy, float & outz, float distmod)
	{
		return CollisionMgr->getObjectHitPos( mapId, x1, y1, z1, x2, y2, z2, outx, outy, outz, distmod );
	}
	
	SHYEMU_INLINE bool IsIndoor(uint32 mapId, float x, float y, float z)
	{
		return CollisionMgr->isInDoors( mapId, x, y, z );
	}

	SHYEMU_INLINE bool IsOutdoor(uint32 mapId, float x, float y, float z)
	{
		return CollisionMgr->isOutDoors( mapId, x, y, z );
	}

	SHYEMU_INLINE float GetHeight(uint32 mapId, float x, float y, float z)
	{
		return CollisionMgr->getHeight( mapId, x, y, z );
	}

	SHYEMU_INLINE bool CheckLOS(uint32 mapId, LocationVector & pos1, LocationVector & pos2)
	{
		return CollisionMgr->isInLineOfSight( mapId, pos1, pos2 );
	}

	SHYEMU_INLINE bool GetFirstPoint(uint32 mapId, LocationVector & pos1, LocationVector & pos2, LocationVector & outvec, float distmod)
	{
		return CollisionMgr->getObjectHitPos( mapId, pos1, pos2, outvec, distmod );
	}

	SHYEMU_INLINE bool IsIndoor(uint32 mapId, LocationVector & pos)
	{
		return CollisionMgr->isInDoors( mapId, pos );
	}

	SHYEMU_INLINE bool IsOutdoor(uint32 mapId, LocationVector & pos)
	{
		return CollisionMgr->isOutDoors( mapId, pos );
	}

	SHYEMU_INLINE float GetHeight(uint32 mapId, LocationVector & pos)
	{
