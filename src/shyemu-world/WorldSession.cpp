...
bool WorldSession::IsLocalized(uint32 id)
{
	Log.Notice("langage", "language = %u", language);
	if( language == id )
		return true;
	else
		return false;
}
const char* WorldSession::LocalizedInstanceSrv(uint32 id)
{
	InstanceStringTable * wst = InstanceStringTableStorage.LookupEntry(id);
	if(!wst){
		memset(szError,0,64);
		sprintf(szError,"ID:%u is a bad InstanceString TEXT!",id);
		return szError;
	}

	LocalizedInstanceStringTable * lpi = (language>0) ? sLocalizationMgr.GetLocalizedInstanceStringTable(id,language):NULL;
	if(lpi)
		return lpi->Text;
	else
		return wst->text;
