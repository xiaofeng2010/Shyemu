...
public:
	...
	SHYEMU_INLINE bool IsInWorld() { return m_mapMgr != NULL; }
	...
	SHYEMU_INLINE const uint64& GetGUID() const { return *((uint64*)m_uint32Values); }
#else
	SHYEMU_INLINE const uint64 GetGUID() const { return GetUInt64Value(0); }
#endif

	SHYEMU_INLINE const WoWGuid& GetNewGUID() const { return m_wowGuid; }
	SHYEMU_INLINE uint32 GetEntry(){return m_uint32Values[3];}

	SHYEMU_INLINE const uint32 GetEntryFromGUID() const
	{
...
*/
	...
	SHYEMU_INLINE const uint32 GetTypeFromGUID() const { return (m_uint32Values[1] & HIGHGUID_TYPE_MASK); }
	SHYEMU_INLINE const uint32 GetUIdFromGUID() const { return (m_uint32Values[0] & LOWGUID_ENTRY_MASK); }
	SHYEMU_INLINE const uint32 GetLowGUID() const { return (m_uint32Values[0]); }

	// type
	SHYEMU_INLINE const uint8& GetTypeId() const { return m_objectTypeId; }
	...
	SHYEMU_INLINE const float& GetPositionX( ) const { return m_position.x; }
	SHYEMU_INLINE const float& GetPositionY( ) const { return m_position.y; }
	SHYEMU_INLINE const float& GetPositionZ( ) const { return m_position.z; }
	SHYEMU_INLINE const float& GetOrientation( ) const { return m_position.o; }
	SHYEMU_INLINE void SetOrientation( float &o ) { m_position.o = o; }

	SHYEMU_INLINE const float& GetSpawnX( ) const { return m_spawnLocation.x; }
	SHYEMU_INLINE const float& GetSpawnY( ) const { return m_spawnLocation.y; }
	SHYEMU_INLINE const float& GetSpawnZ( ) const { return m_spawnLocation.z; }
	SHYEMU_INLINE const float& GetSpawnO( ) const { return m_spawnLocation.o; }

	SHYEMU_INLINE const LocationVector & GetPosition() { return m_position; }
	SHYEMU_INLINE LocationVector & GetPositionNC() { return m_position; }
	SHYEMU_INLINE LocationVector * GetPositionV() { return &m_position; }
	...
	//! Only for MapMgr use
	SHYEMU_INLINE MapCell* GetMapCell() const { return m_mapCell; }
	//! Only for MapMgr use
	SHYEMU_INLINE void SetMapCell(MapCell* cell) { m_mapCell = cell; }
	//! Only for MapMgr use
	SHYEMU_INLINE MapMgr* GetMapMgr() const { return m_mapMgr; }

	SHYEMU_INLINE void SetMapId(uint32 newMap) { m_mapId = newMap; }
	void SetZoneId(uint32 newZone);

	SHYEMU_INLINE const uint32 GetMapId( ) const { return m_mapId; }
	SHYEMU_INLINE const uint32& GetZoneId( ) const { return m_zoneId; }

	//! Get uint32 property
	SHYEMU_INLINE const uint32& GetUInt32Value( uint32 index ) const
	{
...
#else
	SHYEMU_INLINE const uint64& GetUInt64Value( uint32 index ) const
#endif
	{
	...
	//! Get float property
	SHYEMU_INLINE const float& GetFloatValue( uint32 index ) const
	{
		ASSERT( index < m_valuesCount );
	...
	SHYEMU_INLINE uint8 GetByte(uint32 i,uint32 i1)
	{
	...
	SHYEMU_INLINE bool HasByteFlag(uint32 index, uint32 index1, uint8 flag)
	{
		return ((GetByte(index, index1) & flag) != 0);
	}

	SHYEMU_INLINE void SetNewGuid(uint32 Guid)
	{
	...
	SHYEMU_INLINE bool HasFlag( const uint32 index, uint32 flag ) const
	{
		ASSERT( index < m_valuesCount );
	...
	SHYEMU_INLINE const float GetDistanceSq(Object* obj)
	{
		if(obj->GetMapId() != m_mapId) return 40000.0f; //enough for out of range
		return m_position.DistanceSq(obj->GetPosition());
	}

	SHYEMU_INLINE float GetDistanceSq(LocationVector & comp)
	{
		return comp.DistanceSq(m_position);
	}

	SHYEMU_INLINE float CalcDistance(LocationVector & comp)
	{
		return comp.Distance(m_position);
	}

	SHYEMU_INLINE const float GetDistanceSq(float x, float y, float z)
	{
		return m_position.DistanceSq(x, y, z);
	}

	SHYEMU_INLINE const float GetDistance2dSq( Object* obj )
	{
	...
	SHYEMU_INLINE bool IsInRangeSet( Object* pObj )
	{
	...
	SHYEMU_INLINE void AquireInrangeLock(){ m_inrangechangelock.Acquire(); }
	SHYEMU_INLINE void ReleaseInrangeLock(){ m_inrangechangelock.Release(); }

	SHYEMU_INLINE void RemoveInRangeObject( Object* pObj )
	{
		if( pObj == NULL )
			return;
	...
	SHYEMU_INLINE bool HasInRangeObjects()
	{
	...
	SHYEMU_INLINE size_t GetInRangeCount() { return m_objectsInRange.size(); }
	SHYEMU_INLINE size_t GetInRangePlayersCount() { return m_inRangePlayers.size();}
	SHYEMU_INLINE InRangeSet::iterator GetInRangeSetBegin() { return m_objectsInRange.begin(); }
	SHYEMU_INLINE InRangeSet::iterator GetInRangeSetEnd() { return m_objectsInRange.end(); }
	SHYEMU_INLINE InRangeSet::iterator FindInRangeSet(Object * obj) { return m_objectsInRange.find(obj); }
	...
	SHYEMU_INLINE bool RemoveIfInRange( Object * obj )
	{
	...
	SHYEMU_INLINE void AddInRangePlayer( Object * obj )
	{
		m_inRangePlayers.insert( reinterpret_cast< Player* >( obj ) );
	}

	SHYEMU_INLINE void RemoveInRangePlayer( Object * obj )
	{
		m_inRangePlayers.erase( reinterpret_cast< Player* >( obj ) );
	}
	...
	SHYEMU_INLINE std::set<Object*>::iterator GetInRangeSameFactsSetBegin() { return m_sameFactsInRange.begin(); }
	SHYEMU_INLINE std::set<Object*>::iterator GetInRangeSameFactsSetEnd() { return m_sameFactsInRange.end(); }

	bool IsInRangeOppFactSet(Object* pObj) { return (m_oppFactsInRange.count(pObj) > 0); }
	void UpdateOppFactionSet();
	SHYEMU_INLINE size_t GetInRangeOppFactsSize(){ return m_oppFactsInRange.size(); }
	SHYEMU_INLINE std::set<Object*>::iterator GetInRangeOppFactsSetBegin() { return m_oppFactsInRange.begin(); }
	SHYEMU_INLINE std::set<Object*>::iterator GetInRangeOppFactsSetEnd() { return m_oppFactsInRange.end(); }
	SHYEMU_INLINE std::set<Player*>::iterator GetInRangePlayerSetBegin() { return m_inRangePlayers.begin(); }
	SHYEMU_INLINE std::set<Player*>::iterator GetInRangePlayerSetEnd() { return m_inRangePlayers.end(); }
	SHYEMU_INLINE std::set<Player*> * GetInRangePlayerSet() { return &m_inRangePlayers; };

	void __fastcall SendMessageToSetLocalized(WorldPacket *data, bool self,bool myteam_only=false, uint32 IdLocalized = 0);
	void __fastcall SendMessageToSet(WorldPacket *data, bool self,bool myteam_only=false);
	SHYEMU_INLINE void SendMessageToSet(StackBufferBase * data, bool self) { OutPacketToSet(data->GetOpcode(), static_cast<uint16>( data->GetSize() ), data->GetBufferPointer(), self); }
	...
	SHYEMU_INLINE uint16 GetValuesCount() const { return m_valuesCount; }
	...
	SHYEMU_INLINE const uint32 GetPhase() { return m_phase; }
	...
	SHYEMU_INLINE void SetInstanceID(int32 instance) { m_instanceId = instance; }
	SHYEMU_INLINE int32 GetInstanceID() { return m_instanceId; }
...
public:
	...
	bool m_inQueue;
	SHYEMU_INLINE void SetMapMgr(MapMgr * mgr) { m_mapMgr = mgr; }
	...
	//! 
	SHYEMU_INLINE size_t GetInRangeOppFactCount() { return m_oppFactsInRange.size(); }
	...
	SHYEMU_INLINE virtual Group *GetGroup() { return NULL; }
