...
/*	typedef struct { const char *name; int(*mfunc)(lua_State*,GameObject*); } RegType;
	...
	SHYEMU_INLINE GameObjectInfo* GetInfo() { return pInfo; }
	SHYEMU_INLINE void SetInfo(GameObjectInfo * goi) { pInfo = goi; }
   ...
   // void Create ( uint32 mapid, float x, float y, float z, float ang);
	...
	SHYEMU_INLINE bool isQuestGiver()
	{
	...
	SHYEMU_INLINE GameObjectAIScript* GetScript() { return myScript; }

	void TrapSearchTarget();	// Traps need to find targets faster :P

	SHYEMU_INLINE bool HasAI() { return spell != 0; }
	GOSpawn * m_spawn;
	void OnPushToWorld();
	void OnRemoveInRangeObject(Object* pObj);
	void RemoveFromWorld(bool free_guid);

	SHYEMU_INLINE bool CanMine(){return (usage_remaining > 0);}
	SHYEMU_INLINE void UseMine(){ if(usage_remaining) usage_remaining--;}
	...
	SHYEMU_INLINE bool CanFish() { return ( usage_remaining > 0 ); }
	SHYEMU_INLINE void CatchFish() { if ( usage_remaining ) usage_remaining--; }
	...
	SHYEMU_INLINE uint32 GetOverrides() { return m_overrides; }
