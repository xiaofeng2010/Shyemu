...
public:
	...
	/// Creature inventory
	SHYEMU_INLINE uint32 GetItemIdBySlot(uint32 slot) { return m_SellItems->at(slot).itemid; }
	SHYEMU_INLINE uint32 GetItemAmountBySlot(uint32 slot) { return m_SellItems->at(slot).amount; }

	SHYEMU_INLINE bool HasItems() { return ((m_SellItems != NULL) ? true : false); }
	SHYEMU_INLINE CreatureProto* GetProto() { return proto; }
	...
	SHYEMU_INLINE void GetSellItemBySlot(uint32 slot, CreatureItem &ci)
	{
	...
	SHYEMU_INLINE std::vector<CreatureItem>::iterator GetSellItemBegin() { return m_SellItems->begin(); }
	SHYEMU_INLINE std::vector<CreatureItem>::iterator GetSellItemEnd()   { return m_SellItems->end(); }
	SHYEMU_INLINE size_t GetSellItemCount() { return m_SellItems->size(); }
	...
	SHYEMU_INLINE bool isVendor()		const { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_VENDOR ); }
	SHYEMU_INLINE bool isTrainer()		const { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_TRAINER ); }
	SHYEMU_INLINE bool isClass()		const { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_TRAINER_CLASS ); }
	SHYEMU_INLINE bool isProf()			const { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_TRAINER_PROF ); }
	SHYEMU_INLINE bool isQuestGiver()	const { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_QUESTGIVER ); }
	SHYEMU_INLINE bool isGossip()		const { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP ); }
	SHYEMU_INLINE bool isTaxi()			const { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_TAXIVENDOR ); }
	SHYEMU_INLINE bool isCharterGiver()	const { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_ARENACHARTER ); }
	SHYEMU_INLINE bool isGuildBank()	const { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GUILD_BANK ); }
	SHYEMU_INLINE bool isBattleMaster()	const { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_BATTLEFIELDPERSON ); }
	SHYEMU_INLINE bool isBanker()		const { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_BANKER ); }
	SHYEMU_INLINE bool isInnkeeper()	const { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_INNKEEPER ); }
	SHYEMU_INLINE bool isSpiritHealer()	const { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_SPIRITHEALER ); }
	SHYEMU_INLINE bool isTabardDesigner() const { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_TABARDCHANGER ); }
	SHYEMU_INLINE bool isAuctioner()	const { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_AUCTIONEER ); }
	SHYEMU_INLINE bool isStableMaster()	const { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_STABLEMASTER ); }
	SHYEMU_INLINE bool isArmorer()		const { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_ARMORER ); }
	...
	SHYEMU_INLINE uint32 GetSQL_id() { return spawnid; };
	...
	SHYEMU_INLINE Player *GetTotemOwner() { return totemOwner; }
	SHYEMU_INLINE void SetTotemOwner(Player *owner) { totemOwner = owner; }
	SHYEMU_INLINE uint32 GetTotemSlot() { return totemSlot; }
	SHYEMU_INLINE void SetTotemSlot(uint32 slot) { totemSlot = slot; }

	virtual Group *GetGroup();

	SHYEMU_INLINE bool IsPickPocketed() { return m_PickPocketed; }
	SHYEMU_INLINE void SetPickPocketed(bool val = true) { m_PickPocketed = val; }

	SHYEMU_INLINE CreatureAIScript * GetScript() { return _myScriptClass; }
	void LoadScript();
	...
	SHYEMU_INLINE CreatureInfo *GetCreatureInfo() { return creature_info; }
	SHYEMU_INLINE void SetCreatureInfo(CreatureInfo *ci) { creature_info = ci; }
	SHYEMU_INLINE Trainer* GetTrainer() { return mTrainer; }
	void RegenerateFocus();

	CreatureFamilyEntry * myFamily;
	SHYEMU_INLINE bool IsTotem() { return totemOwner != 0 && totemSlot != -1; }

	SHYEMU_INLINE bool IsExotic()
	{
