...
SHYEMU_INLINE std::string MyConvertIntToString(const int arg)
{
	stringstream out;
	out << arg;
	return out.str();
...
SHYEMU_INLINE std::string MyConvertFloatToString(const float arg)
{
	stringstream out;
	out << arg;
	return out.str();
...
bool ChatHandler::HandleQuestFinishCommand(const char * args, WorldSession * m_session)
{
	...
	if(qst)
	{
		...
		else
		{
			...
			if ( qst->is_repeatable == shyemu_QUEST_REPEATABLE_DAILY )
