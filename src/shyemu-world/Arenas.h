...
public:
	...
	bool CountPerTeamEnough(int32 team)
	{
		size_t c0 = m_players[team].size() + m_pendPlayers[team].size();
		if(m_started)
			return true;
	...
	void Finish();
	SHYEMU_INLINE uint8 Rated() { return rated_match; }
	SHYEMU_INLINE uint32 GetArenaTeamType() { return m_arenateamtype; }
	SHYEMU_INLINE ArenaTeam ** GetTeams() { return m_teams; }
