...
public:
	...
	SHYEMU_INLINE void SendPacket(WorldPacket* packet) { if(!packet) return; OutPacket(packet->GetOpcode(), packet->size(), (packet->size() ? (const void*)packet->contents() : NULL)); }
	SHYEMU_INLINE void SendPacket(StackBufferBase * packet) { if(!packet) return; OutPacket(packet->GetOpcode(), packet->GetSize(), (packet->GetSize() ? (const void*)packet->GetBufferPointer() : NULL)); }
	...
	SHYEMU_INLINE void SetSession(WorldSession * session) { mSession = session; }
	SHYEMU_INLINE WorldSession * GetSession() { return mSession; }
	bool Authed;
