...
public:
	...
	SHYEMU_INLINE string GetNameString() { return name; }
	SHYEMU_INLINE const char* GetName() { return name.c_str(); }
	SHYEMU_INLINE MapEntry* GetDBCEntry() { return me; }

	SHYEMU_INLINE CellSpawns *GetSpawnsList(uint32 cellx,uint32 celly)
	{
		ASSERT(cellx < _sizeX);
		ASSERT(celly < _sizeY);
	...
	SHYEMU_INLINE CellSpawns * GetSpawnsListAndCreate(uint32 cellx, uint32 celly)
	{
		ASSERT(cellx < _sizeX);
		ASSERT(celly < _sizeY);
	...
	SHYEMU_INLINE float  GetLandHeight(float x, float y)
	{ 
		if(_terrain)
		{
	...
	SHYEMU_INLINE float  GetWaterHeight(float x, float y) 
	{ 
		if(_terrain)
		{ 
	...
	SHYEMU_INLINE uint8  GetWaterType(float x, float y)
	{
		if(_terrain)
		{ 
	...
	SHYEMU_INLINE uint8  GetWalkableState(float x, float y)
	{
		if(_terrain)
		{ 
	...
	SHYEMU_INLINE uint16 GetAreaID(float x, float y)
	{
		if(_terrain)
		{ 
	...
	SHYEMU_INLINE void CellGoneActive(uint32 x, uint32 y)
	{ 
		if(_terrain)
		{ 
	...
	SHYEMU_INLINE void CellGoneIdle(uint32 x,uint32 y)
	{ 
		if(_terrain)
		{ 
