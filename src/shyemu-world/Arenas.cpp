...
void Arena::OnCreate()
{
	...
	case 559: {
		...
		case 617:
		{
			obj = SpawnGameObject(192643, 617, 1247.620361f, 764.673401f, 20.5f, 0.0f, 32, 1375, 2.0f);
			if(obj)
			{
			...
			obj = SpawnGameObject(192642, 617, 1335.028198f, 817.274658f, 20.5f, 0.0f, 32, 1375, 2.0f);
			if(obj)
			{
				obj->SetByte(GAMEOBJECT_BYTES_1,GAMEOBJECT_BYTES_STATE, 1);
				obj->SetByte(GAMEOBJECT_BYTES_1,GAMEOBJECT_BYTES_ANIMPROGRESS, 100);
	...
	case 618:
		{
			/*TODO:
			 * - fix these elevators, i ran out of ideas, lost few hours... -> Mettre le flag a 40
			 * - spawn rest of those stupid gameobj, im too lazy to get their coords
			 * - that moving things shouldn't move all the time...
			 */

			//elevators
			obj = SpawnGameObject(194030, 618, 763.25f, -293.8f, 26.0f, 0.0f, 40, 1375, 1.0f);
			if(obj)
			{
			...
			obj = SpawnGameObject(194031, 618, 763.25f, -273.8f, 26.0f, 0.0f, 40, 1375, 1.0f);
			if(obj)
			{
				obj->SetByte(GAMEOBJECT_BYTES_1, GAMEOBJECT_BYTES_TYPE_ID, 5);
				obj->PushToWorld(m_mapMgr);
			...
			obj = SpawnGameObject(193458, 618, 763.761f, -306.230f, 26.0f, 0.0f, 40, 1375, 1.0f);
			if(obj)
				obj->PushToWorld(m_mapMgr);

			obj = SpawnGameObject(193461, 618, 802.313f, -284.349f, 24.6f, 0.0f, 40, 1375, 1.0f);
			if(obj)
				obj->PushToWorld(m_mapMgr);
...
void Arena::OnStart()
{
	...
	for(i = 0; i < 2; ++i) {
		for(set<Player*>::iterator itr = m_players[i].begin(); itr != m_players[i].end(); ++itr) {
			Player *plr = *itr;
			static_cast<Player*>((*itr))->RemoveAura(32728);
...
LocationVector Arena::GetStartingCoords(uint32 Team)
{
	...
	*/
	...
	case 617:
		{
			if(Team)
				return LocationVector(1227.096f, 764.634f, 15.5f);
			else
				return LocationVector(1360.581f, 817.17f, 15.5f);
		}break;
	/*	The Ring of Valor */ 
	case 618:
		{
			//temp workaround
			if(Team)
				return LocationVector(763.56f, -274.0f, 28.27f/*3.55*/);
			else
