...
////////////AI
	...
	if(!(proto->isTrainingDummy))
		m_aiInterface->SetAllowedToEnterCombat(proto->m_isAllToEnterCombat);
	else
...
//Used for Infernal
void Creature::ExpireInferno(Player * plr)
{
	static_cast<Unit*>(this)->m_special_state &= ~UNIT_STATE_CHARM;
	static_cast<Unit*>(this)->SetUInt32Value( UNIT_FIELD_FACTIONTEMPLATE, static_cast<Unit*>(this)->GetCharmTempVal() );
	static_cast<Unit*>(this)->_setFaction();
	static_cast<Unit*>(this)->GetAIInterface()->WipeHateList();
	static_cast<Unit*>(this)->GetAIInterface()->WipeTargetList();
	static_cast<Unit*>(this)->UpdateOppFactionSet();
	static_cast<Unit*>(this)->GetAIInterface()->Init(static_cast<Unit*>(this), AITYPE_AGRO, MOVEMENTTYPE_NONE);
	static_cast<Unit*>(this)->SetUInt64Value(UNIT_FIELD_CHARMEDBY, 0);

	if( plr != NULL && plr->GetSession() != NULL ) // crashfix
	{
		plr->SetUInt64Value(UNIT_FIELD_CHARM, 0);
