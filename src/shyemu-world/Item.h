...
public:
	...
	SHYEMU_INLINE ItemPrototype* GetProto() const { return m_itemProto; }
	SHYEMU_INLINE void SetProto( ItemPrototype* pr ) { m_itemProto = pr; }
	SHYEMU_INLINE bool IsPotion()
	{
		return (GetProto()->Class == ITEM_CLASS_CONSUMABLE && GetProto()->SubClass == ITEM_SUBCLASS_CONSUMABLE_POTION) ? true : false; 
	}

	SHYEMU_INLINE Player* GetOwner() const { return m_owner; }
	void SetOwner( Player* owner );

	SHYEMU_INLINE bool IsContainer(){ return ( m_objectTypeId == TYPEID_CONTAINER ) ? true : false; }
	...
	SHYEMU_INLINE time_t GetEnchantmentApplytime( uint32 slot )
	{
	...
	SHYEMU_INLINE void SetCount( uint32 amt ) { SetUInt32Value( ITEM_FIELD_STACK_COUNT, amt ); }
	SHYEMU_INLINE void SetDurability( uint32 Value ) { SetUInt32Value(ITEM_FIELD_DURABILITY, Value ); };
	SHYEMU_INLINE void SetDurabilityToMax() { SetUInt32Value( ITEM_FIELD_DURABILITY, GetUInt32Value( ITEM_FIELD_MAXDURABILITY ) ); }
	SHYEMU_INLINE uint32 GetDurability() { return GetUInt32Value( ITEM_FIELD_DURABILITY ); }
	SHYEMU_INLINE uint32 GetDurabilityMax() { return GetUInt32Value( ITEM_FIELD_MAXDURABILITY ); }
	SHYEMU_INLINE bool IsAmmoBag() { return (m_itemProto->Class == ITEM_CLASS_QUIVER); }
	...
	SHYEMU_INLINE uint32 GetItemRandomPropertyId() const { return m_uint32Values[ITEM_FIELD_RANDOM_PROPERTIES_ID]; }
	SHYEMU_INLINE uint32 GetItemRandomSuffixFactor() { return m_uint32Values[ITEM_FIELD_PROPERTY_SEED]; }
	static uint32 GenerateRandomSuffixFactor( ItemPrototype* m_itemProto );

	SHYEMU_INLINE void SetRandomProperty( uint32 id )
	{
		SetUInt32Value( ITEM_FIELD_RANDOM_PROPERTIES_ID, id );
		random_prop = id;
	}

	SHYEMU_INLINE void SetRandomSuffix( uint32 id )
	{
		int32 r_id = -(int32(id));
