...
public:
	SHYEMU_INLINE uint32 getHordePlayerCount() { return HordePlayers; }
	SHYEMU_INLINE uint32 getAlliancePlayerCount() { return AlliancePlayers; }
	SHYEMU_INLINE uint32 getPlayerCount() { return (HordePlayers + AlliancePlayers); }
	SHYEMU_INLINE void resetPlayerCount() { HordePlayers = AlliancePlayers = 0; }
	SHYEMU_INLINE void incrementPlayerCount(uint32 faction)
	{
		if( faction == 1 )
			HordePlayers++;
		else
	...
	SHYEMU_INLINE void decrementPlayerCount(uint32 faction)
	{
		if( faction == 1 )
			HordePlayers--;
		else
...
public:
	...
	SHYEMU_INLINE size_t GetSessionCount() const { return m_sessions.size(); }
	uint32 GetNonGmSessionCount();
	SHYEMU_INLINE size_t GetQueueCount() { return mQueuedSessions.size(); }
	void GetStats(uint32 * GMCount, float * AverageLatency);

	SHYEMU_INLINE uint32 GetPlayerLimit() const { return m_playerLimit; }
	void SetPlayerLimit(uint32 limit) { m_playerLimit = limit; }

	SHYEMU_INLINE bool getAllowMovement() const { return m_allowMovement; }
	void SetAllowMovement(bool allow) { m_allowMovement = allow; }
	SHYEMU_INLINE bool getGMTicketStatus() { return m_gmTicketSystem; };
	...
	SHYEMU_INLINE std::string getGmClientChannel() { return GmClientChannel; }

	void SetMotd(const char *motd) { m_motd = motd; }
	SHYEMU_INLINE const char* GetMotd() const { return m_motd.c_str(); }

	SHYEMU_INLINE time_t GetGameTime() const { return m_gameTime; }
	...
	SHYEMU_INLINE void SetStartTime(uint32 val) { m_StartTime = val; }
	SHYEMU_INLINE uint32 GetUptime(void) { return (uint32)UNIXTIME - m_StartTime; }
	SHYEMU_INLINE uint32 GetStartTime(void) { return m_StartTime; }
	std::string GetUptimeString();
	// cebernic: textfilter,no fast,but works:D ...
	SHYEMU_INLINE std::string SessionLocalizedTextFilter(WorldSession* _session,const char*text)
	{
	...
	SHYEMU_INLINE void setRate(int index,float value)
	{
		regen_values[index]=value;
	}

	SHYEMU_INLINE float getRate(int index)
	{
		return regen_values[index];
	}
	
	SHYEMU_INLINE uint32 getIntRate(int index)
	{
		return int_rates[index];
	}

	SHYEMU_INLINE void setIntRate(int index, uint32 value)
	{
	...
	SHYEMU_INLINE uint32 GetTimeOut(){return TimeOut;}
