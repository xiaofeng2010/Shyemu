...
public:
    ...
    SHYEMU_INLINE SpellEntry* GetSpellProto() const { return m_spellProto; }
    SHYEMU_INLINE uint32 GetSpellId() const {return m_spellProto->Id; }
    SHYEMU_INLINE bool IsPassive(){ if(!m_spellProto) return false; return (m_spellProto->Attributes & ATTRIBUTES_PASSIVE && !m_areaAura);}

    SHYEMU_INLINE int32 GetDuration() const { return m_duration; }
    ...
    SHYEMU_INLINE uint16 GetAuraSlot() const { return m_auraSlot; }
	void SetAuraSlot( uint16 slot ) { m_auraSlot = slot; }

	SHYEMU_INLINE bool IsPositive() { return m_positive>0; }
	void SetNegative(signed char value=1) { m_positive -= value; }
	void SetPositive(signed char value=1) { m_positive += value; }

	Object* GetCaster();
	SHYEMU_INLINE uint64 GetCasterGUID(){return m_casterGuid;}
	Unit* GetUnitCaster();
	SHYEMU_INLINE Unit* GetTarget() { return m_target; }
	...
	SHYEMU_INLINE uint32 GetTimeLeft() 
	{
	...
	void SpellAuraConvertRuneInDeath(bool apply);
	void SpellAuraAuraCloneCaster(bool apply);
...
//	void SpellAuraIncreaseHealingByInt(bool apply);
	...
	SHYEMU_INLINE void SendPeriodicHealAuraLog(uint32 amt)
	{
		WorldPacket data(32);
	...
	// log message's
	SHYEMU_INLINE void SendPeriodicAuraLog(Unit * Caster, Unit * Target, uint32 SpellID, uint32 School, uint32 Amount, uint32 abs_dmg, uint32 resisted_damage, uint32 Flags)
	{
	...
	SHYEMU_INLINE bool IsCombatStateAffecting()
	{
...
private:
	...
	SHYEMU_INLINE bool IsInrange(float x1,float y1, float z1, Object * o,float square_r)
	{
		float t;
		float r;
		t=x1-o->GetPositionX();
...
public:
	...
	SHYEMU_INLINE bool IsInterrupted() { return ( m_interrupted >= 0 ); }
};
