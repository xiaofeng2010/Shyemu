...
//		SetNextTarget(FindTargetForSpell(m_nextSpell));
	...
	if( cansee && GetNextTarget() && GetNextTarget()->isAlive() && m_AIState != STATE_EVADE && !m_Unit->IsCasting() )
	{
		if( agent == AGENT_NULL || ( m_AIType == AITYPE_PET && !m_nextSpell ) ) // allow pets autocast
		{
			...
			if(m_canFlee && !m_hasFleed && m_Unit->GetHealthPct() < 10.0f)
				agent = AGENT_FLEE;
		...
		case AGENT_FLEE:
			{
				...
				m_moveRun = false;
				if(m_fleeTimer == 0)
					m_fleeTimer = getMSTime() + m_FleeDuration;

				UnitToFear = GetNextTarget();

				//Log.Notice("Fuite!!!!!!", "Fuite");
				...
				string msg = "est pris de panique!";
				...
				//	TO_PLAYER(m_nextTarget)->SendMessageToSet(&data, true);
				m_Unit->CastSpell(m_Unit, 33829, true);
				m_Unit->SendChatMessage(CHAT_MSG_MONSTER_EMOTE, LANG_UNIVERSAL, "%s est pris de panique!", TO_CREATURE( m_Unit )->GetCreatureInfo()->Name);
				m_hasFleed = true;
