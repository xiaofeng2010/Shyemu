...
public:
	...
	SHYEMU_INLINE GroupMembersSet::iterator GetGroupMembersBegin(void) { return m_GroupMembers.begin(); }
	SHYEMU_INLINE GroupMembersSet::iterator GetGroupMembersEnd(void)   { return m_GroupMembers.end();   }

	bool AddPlayer(PlayerInfo * info);
	void RemovePlayer(PlayerInfo * info);
	
	SHYEMU_INLINE bool IsFull(void)				{ return m_GroupMembers.size() >= MAX_GROUP_SIZE_PARTY; }
	SHYEMU_INLINE size_t GetMemberCount(void)		{ return m_GroupMembers.size(); }
	
	SHYEMU_INLINE uint32 GetID(void)			   { return m_Id; }
	SHYEMU_INLINE void SetID(uint32 newid)		 { m_Id = newid; }

	SHYEMU_INLINE void   SetParent(Group* parent)  { m_Parent = parent; }
	SHYEMU_INLINE Group* GetParent(void)		   { return m_Parent; }
...
public:
	...
	SHYEMU_INLINE void SendPacketToAll(WorldPacket *packet) { SendPacketToAllButOne(packet, NULL); }
	void SendPacketToAllButOne(WorldPacket *packet, Player *pSkipTarget);

	SHYEMU_INLINE void OutPacketToAll(uint16 op, uint16 len, const void* data) { OutPacketToAllButOne(op, len, data, NULL); }
	...
	// Accessing functions
	SHYEMU_INLINE SubGroup* GetSubGroup(uint32 Id)
	{
		if(Id >= 8)
			return 0;
	...
	SHYEMU_INLINE uint32 GetSubGroupCount(void) { return m_SubGroupCount; }

	SHYEMU_INLINE uint8 GetMethod(void) { return m_LootMethod; }
	SHYEMU_INLINE uint16 GetThreshold(void) { return m_LootThreshold; }
	SHYEMU_INLINE PlayerInfo* GetLeader(void) { return m_Leader; }
	SHYEMU_INLINE PlayerInfo* GetLooter(void) { return m_Looter; }

	void MovePlayer(PlayerInfo* info, uint8 subgroup);

	bool HasMember(Player *pPlayer);
	bool HasMember(PlayerInfo * info);
	SHYEMU_INLINE uint32 MemberCount(void) { return m_MemberCount; }
	SHYEMU_INLINE bool IsFull() { return ((m_GroupType == GROUP_TYPE_PARTY && m_MemberCount >= MAX_GROUP_SIZE_PARTY) || (m_GroupType == GROUP_TYPE_RAID && m_MemberCount >= MAX_GROUP_SIZE_RAID)); }
	...
	SHYEMU_INLINE uint8 GetGroupType() { return m_GroupType; }
	SHYEMU_INLINE uint32 GetID() { return m_Id; }
	...
	SHYEMU_INLINE Mutex& getLock() { return m_groupLock; }
	SHYEMU_INLINE void Lock() { m_groupLock.Acquire(); }
	SHYEMU_INLINE void Unlock() { return m_groupLock.Release(); }
	bool m_isqueued;
	...
	SHYEMU_INLINE PlayerInfo * GetAssistantLeader() { return m_assistantLeader; }
	SHYEMU_INLINE PlayerInfo * GetMainTank() { return m_mainTank; }
	SHYEMU_INLINE PlayerInfo * GetMainAssist() { return m_mainAssist; }
