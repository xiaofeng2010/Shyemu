...
/*
SHYEMU_INLINE void ApplyFloatSM(float ** m,float v,uint32 mask, float def)
{
	if(*m == 0)
	{
...
/*
SHYEMU_INLINE void ApplyFloatPSM(float ** m,int32 v,uint32 mask, float def)
{
	if(*m == 0)
	{
		*m = new float[SPELL_GROUPS];
...
void Aura::SpellAuraPeriodicDamage(bool apply)
{
	if(apply)
	{
		...
		case SPELL_HASH_EXPLOSIVE_SHOT:
			{
				c->Strike( m_target, RANGED, m_spellProto, dmg+c->GetAP()*0.14, 0, 0, false, false);
			}break;
		case SPELL_HASH_BLACK_ARROW:
			dmg += c->GetAP()*0.1;break;
		}
                   ...
                   if(circle && sp && m_target->CalcDistance(circle) <= 40.0f )        
                       {
                               ...
                               //{
                                       ...
                                       se->Id = 62388;
                                       test->Init(se, 1000, m_target, m_target, true);
                                       test->m_visualSlot = 35;
                                       m_target->ModVisualAuraStackCount(test, 1);
                                       delete test;
                                       delete se;
                               //}
                       }
                       ...
                       {
                               //if(m_target->m_auraStackCount[35] >= 1)
                               //{
                                       Aura * test = new Aura();
                                       SpellEntry * se = new SpellEntry();
                                       se->Id = 62388;
...
void Aura::SpellAuraDummy(bool apply)
{
	...
	case SPELL_HASH_HAUNT:
		{
			if(!apply)
			{
				if(GetUnitCaster())
				{
					Log.Notice("spell", "m_lastHauntInitialDamage = %u", m_target->m_lastHauntInitialDamage);
					GetUnitCaster()->Heal( GetUnitCaster(), GetSpellId(), m_target->m_lastHauntInitialDamage );
				}
			}
		}break;
	}
	...
	switch(GetSpellId())
	{
		case 46432:
		{
			if(!apply)
			{
				...
				{
					if(QuestLogEntry * en =  TO_PLAYER(GetUnitCaster())->GetQuestLogForEntry(11896))
	...
	case 46374:
		{
			if(apply)
			{
					if(m_target->IsCreature() && m_target->GetEntry() == 24601 && m_target->IsDead())
					{
						Player* Plr = GetUnitCaster()->GetMapMgr()->GetPlayer(GetUnitCaster()->GetUInt64Value(UNIT_FIELD_CREATEDBY));
						if(Plr)
						{
							if(QuestLogEntry * en =  Plr->GetQuestLogForEntry(11893))
								if(en->GetMobCount(0) < en->GetQuest()->required_mobcount[0])
								{
									en->SetMobCount( 0, en->GetMobCount(0)+ 1 );
									en->SendUpdateAddKill(0);
	...
	case 47875:
		{
			...
			if(GetUnitCaster()->IsPlayer())
			{
				if(QuestLogEntry * en = TO_PLAYER(GetUnitCaster())->GetQuestLogForEntry(11690))
				{
					en->SetMobCount( 0, en->GetMobCount(0)+1 );
					en->SendUpdateAddKill(0);
					en->UpdatePlayerFields();
	...
	case 31606:
	{
		if(apply)
		{
			TO_PLAYER(GetUnitCaster())->TaxiStart(sTaxiMgr.GetTaxiPath(95, 96), 20013, 0);
		}
		else
		{
		}
		...
		case 6495:
		{
			Creature * totem = static_cast<Player*>(GetUnitCaster())->GetTotemSentinelle;
				if(!totem)
					return;
			if(apply)
			{
				GetUnitCaster()->SetUInt64Value(PLAYER_FARSIGHT, totem->GetGUID());
				GetUnitCaster()->GetMapMgr()->ChangeFarsightLocation(static_cast<Player*>(GetUnitCaster()), (DynamicObject*)totem);
			}
			else
			{
				GetUnitCaster()->SetUInt64Value(PLAYER_FARSIGHT, 0);
				GetUnitCaster()->GetMapMgr()->ChangeFarsightLocation(static_cast<Player*>(GetUnitCaster()), (DynamicObject*)totem);
			}
		};
	case 570:   // far sight
	...
	case 21171:
		{
			//Log.Notice("Spell", "Eagle Eye");
			if(!apply)
			{
				if(!GetUnitCaster() || !GetUnitCaster()->IsInWorld())
					return;
				DynamicObject * farsight = GetUnitCaster()->GetMapMgr()->GetDynamicObject(p_target->GetUInt32Value(PLAYER_FARSIGHT));
				GetUnitCaster()->SetUInt64Value(PLAYER_FARSIGHT, 0);
				GetUnitCaster()->GetMapMgr()->ChangeFarsightLocation(static_cast<Player*>(GetUnitCaster()), farsight);
				if(farsight)
				{
					farsight->Remove();
				}
	...
	case 44457:
		{
			for(std::set<Object*>::iterator itr = GetUnitCaster()->GetInRangeSetBegin(); itr != GetUnitCaster()->GetInRangeSetEnd(); ++itr)
			{
				if((*itr)->GetTypeId() != TYPEID_UNIT )
					continue;

				if( !(GetUnitCaster()->GetPhase() & (*itr)->GetPhase()) ) //Can't see, no threat
					continue;

				if((*itr)->CalcDistance(GetUnitCaster()) <= 10 && isAttackable((*itr), GetUnitCaster()))
					GetUnitCaster()->DealDamage( (Unit*)(*itr), 306, 0, 0, GetSpellId() );
			}
		}break;
	case 55359:
		{
			for(std::set<Object*>::iterator itr = GetUnitCaster()->GetInRangeSetBegin(); itr != GetUnitCaster()->GetInRangeSetEnd(); ++itr)
			{
				...
				if((*itr)->CalcDistance(GetUnitCaster()) <= 10 && isAttackable((*itr), GetUnitCaster()))
					GetUnitCaster()->DealDamage( (Unit*)(*itr), 512, 0, 0, GetSpellId() );
			}
		}break;
	case 55360:
		{
			for(std::set<Object*>::iterator itr = GetUnitCaster()->GetInRangeSetBegin(); itr != GetUnitCaster()->GetInRangeSetEnd(); ++itr)
			{
				...
				if((*itr)->CalcDistance(GetUnitCaster()) <= 10 && isAttackable((*itr), GetUnitCaster()))
					GetUnitCaster()->DealDamage( (Unit*)(*itr), 690, 0, 0, GetSpellId() );
			}
		}break;
	case 58597://Bouclier saint
		{
	...
	case 53736:
		{
			if(apply)
			{
				if(m_target->GetAuraStackCount(53736) == 5)
				{
					Item * it = static_cast<Player*>(GetUnitCaster())->GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_MAINHAND);
					if(it && it->GetProto())
					{
						uint32 dmg = it->GetProto()->Damage->Max;
						dmg *= 0.33;
						GetUnitCaster()->DealDamage( m_target, dmg, 0, 0, m_spellProto->Id );
					}
				}
			}
		}break;
	//Graine de corruption rang 1 2 3
	case 27243:
		{
			GetUnitCaster()->CastSpellAoF(GetUnitCaster()->GetPositionX(), GetUnitCaster()->GetPositionY(), GetUnitCaster()->GetPositionZ(), dbcSpell.LookupEntryForced(43991), false);
		}break;
	case 47835:
		{
			GetUnitCaster()->CastSpellAoF(GetUnitCaster()->GetPositionX(), GetUnitCaster()->GetPositionY(), GetUnitCaster()->GetPositionZ(), dbcSpell.LookupEntryForced(47831), false);
		}break;
	case 47836:
		{
			GetUnitCaster()->CastSpellAoF(GetUnitCaster()->GetPositionX(), GetUnitCaster()->GetPositionY(), GetUnitCaster()->GetPositionZ(), dbcSpell.LookupEntryForced(47834), false);
		}break;
	case 44394:
		{
			if(apply)
			{
				Log.Notice("dude", "m_lastAbsorbForAbsorbIncanter = %u", GetUnitCaster()->m_lastAbsorbForAbsorbIncanter);
				uint32 amount = GetUnitCaster()->m_lastAbsorbForAbsorbIncanter*0.05;
				uint32 maxAmount = GetUnitCaster()->GetUInt32Value(UNIT_FIELD_MAXHEALTH)*0.05;
				if(amount > maxAmount)
					amount = maxAmount;
				for(uint32 x =0; x<7; x++)
				{
					GetUnitCaster()->ModUnsigned32Value( PLAYER_FIELD_MOD_DAMAGE_DONE_POS + x, amount );
				}
			}
			else
			{
				uint32 amount = (uint32)GetUnitCaster()->m_lastAbsorbForAbsorbIncanter*0.05;
				uint32 maxAmount = (uint32)GetUnitCaster()->GetUInt32Value(UNIT_FIELD_MAXHEALTH)*0.05;
				if(amount > maxAmount)
					amount = maxAmount;
				for(uint32 x =0; x<7; x++)
				{
	...
	case 44395:
		{
			if(apply)
			{
				Log.Notice("dude", "m_lastAbsorbForAbsorbIncanter = %u", GetUnitCaster()->m_lastAbsorbForAbsorbIncanter);
				uint32 amount = (uint32)GetUnitCaster()->m_lastAbsorbForAbsorbIncanter*0.05;
				uint32 maxAmount = (uint32)GetUnitCaster()->GetUInt32Value(UNIT_FIELD_MAXHEALTH)*0.1;
				if(amount > maxAmount)
					amount = maxAmount;
				for(uint32 x =0; x<7; x++)
				{
	...
	case 44396:
		{
			if(apply)
			{
				Log.Notice("dude", "m_lastAbsorbForAbsorbIncanter = %u", GetUnitCaster()->m_lastAbsorbForAbsorbIncanter);
				uint32 amount = (uint32)GetUnitCaster()->m_lastAbsorbForAbsorbIncanter*0.05;
				uint32 maxAmount = (uint32)GetUnitCaster()->GetUInt32Value(UNIT_FIELD_MAXHEALTH)*0.15;
				if(amount > maxAmount)
					amount = maxAmount;
				for(uint32 x =0; x<7; x++)
				{
	...
	case 48018:
		{
			if(apply)
			{
				...
				uint32 slot= 1;
				GoSummon = GetUnitCaster()->m_ObjectSlots[slot] ? GetUnitCaster()->GetMapMgr()->GetGameObject(GetUnitCaster()->m_ObjectSlots[slot]) : 0;
				GetUnitCaster()->m_ObjectSlots[slot] = 0;

				if( GoSummon )
				{
					if(GoSummon->GetInstanceID() != GetUnitCaster()->GetInstanceID())
				...
				// spawn a new one	
				GoSummon = GetUnitCaster()->GetMapMgr()->CreateGameObject(191083);
				if(! GoSummon->CreateFromProto(191083, GetUnitCaster()->GetMapId(), GetUnitCaster()->GetPositionX(), GetUnitCaster()->GetPositionY(), GetUnitCaster()->GetPositionZ(), GetUnitCaster()->GetOrientation() ))
				{
					delete GoSummon;
					return;
				}

				GoSummon->SetUInt32Value(GAMEOBJECT_LEVEL, GetUnitCaster()->getLevel());
				GoSummon->SetUInt64Value(OBJECT_FIELD_CREATED_BY, GetUnitCaster()->GetGUID());
				GoSummon->SetInstanceID(GetUnitCaster()->GetInstanceID());
			
				sEventMgr.AddEvent(GoSummon, &GameObject::ExpireAndDelete, EVENT_GAMEOBJECT_EXPIRE, GetDuration(), 1,0);
			
				GoSummon->PushToWorld(GetUnitCaster()->GetMapMgr());
				GoSummon->SetSummoned(GetUnitCaster());
				GetUnitCaster()->m_ObjectSlots[slot] = GoSummon->GetUIdFromGUID();
			...
			else
			{
				...
				if(m_target->m_ObjectSlots[1] && m_target->GetMapMgr()->GetGameObject(m_target->m_ObjectSlots[1]))
					m_target->GetMapMgr()->GetGameObject(m_target->m_ObjectSlots[1])->Delete();
			}
		}break;
...
void Aura::SpellAuraModCharm(bool apply)
{
	...
	if( apply )
	{
		...
		m_target->GetAIInterface()->SetUnitToFollow(ucaster);
		m_target->GetAIInterface()->SetUnitToFollowAngle(3.14f);
		m_target->GetAIInterface()->SetFollowDistance(3.0f);
...
void Aura::SpellAuraPeriodicHeal( bool apply )
{
	if( apply )
	{
		...
		{
			if(c->HasAura(53503))
				val = m_target->m_LastHealAmount*0.6;
			else if(c->HasAura(53502))
				val = m_target->m_LastHealAmount*0.4;
			else if(c->HasAura(53501))
				val = m_target->m_LastHealAmount*0.2;
			m_target->m_LastHealAmount = 0;
		}
		if (val > 0) {
...
void Aura::SpellAuraModStun(bool apply)
{
	...
	if(!apply)
	{
		if( GetSpellProto()->NameHash == SPELL_HASH_WYVERN_STING )
		{
			...
			case 19386:
				{
					GetUnitCaster()->CastSpell( m_target, 24131, true);
				}break;
			case 24132:
				{
					GetUnitCaster()->CastSpell( m_target, 24134, true);
				}break;
			case 24133:
				{
					GetUnitCaster()->CastSpell( m_target, 24135, true);
				}break;
			case 27068:
				{
					GetUnitCaster()->CastSpell( m_target, 27069, true);
				}break;
			case 49011:
				{
					GetUnitCaster()->CastSpell( m_target, 49009, true);
				}break;
			case 49012:
				{
					GetUnitCaster()->CastSpell( m_target, 49010, true);
				}break;
			}
		}
	}
...
void Aura::SpellAuraModDamageDone(bool apply)
{
	...
	if(GetSpellProto()->NameHash == SPELL_HASH_FLAMETONGUE_TOTEM)
		m_target->RemoveAuraByNameHash(SPELL_HASH_FLAMETONGUE_TOTEM);
	if( m_target->IsPlayer() )
	{
		uint32 index;
...
void Aura::SpellAuraModShapeshift(bool apply)
{
	...
	if( GetUnitCaster()->HasAurasWithNameHash(SPELL_HASH_STARFALL))
		GetUnitCaster()->RemoveAuraByNameHash(SPELL_HASH_STARFALL);
	uint32 spellId = 0;
	uint32 spellId2 = 0;
	uint32 modelId = 0;
	if(m_target->HasAura(34123))
	...
	case FORM_TREE:
		{
			...
			spellId = 5420;//3122;
			//spellId2 = 34123;
			if(GetUnitCaster()->GetGroup())
			{
				for(std::set<Object*>::iterator itr = GetUnitCaster()->GetInRangeSetBegin(), i2; itr != GetUnitCaster()->GetInRangeSetEnd(); )
				{
					i2 = itr++;
					...
					if((*i2)->GetGroup() == GetUnitCaster()->GetGroup() && static_cast<Unit*>((*i2)) != GetUnitCaster())
					{	
						Log.Notice("group is ok", "yep");
						GetUnitCaster()->CastSpell(static_cast<Unit*>((*i2)), 34123, true);
					}
				}
			}
			static_cast< Player* >( GetUnitCaster() )->SetUInt32Value(UNIT_FIELD_DISPLAYID, modelId);
			//static_cast< Player* >( GetUnitCaster() )->EventModelChange();
		} break;
	case FORM_TRAVEL:
		{//druid
			freeMovements = true;
	...
	if(GetUnitCaster()->HasAura(48411) || GetUnitCaster()->HasAura(48412))
	{		
			if(apply)
			{
				Player* mPlayer = static_cast< Player* >( GetUnitCaster() );
				switch(mod->m_miscValue)
				{
				case 2:
					mPlayer->CastSpell(mPlayer, 48422, true);break;
				case FORM_CAT:
					mPlayer->CastSpell(mPlayer, 48420, true);break;
				case FORM_BEAR:
				case FORM_DIREBEAR:
					mPlayer->CastSpell(mPlayer, 48418, true);break;
				case FORM_MOONKIN:
					mPlayer->CastSpell(mPlayer, 48421, true);break;
				default:
					{
					}break;
				}	
			...
			else
			{
				...
				{
				case 2:
					mPlayer->RemoveAura(48422);break;
				case FORM_CAT:
					mPlayer->RemoveAura(48420);break;
				case FORM_BEAR:
				case FORM_DIREBEAR:
					mPlayer->RemoveAura(48418);break;
				case FORM_MOONKIN:
					mPlayer->RemoveAura(48421);break;
				default:
					{
					}break;
				}	
	...
	if (m_target->IsPlayer())
	{
		if(apply)
		{
			...
			switch(GetSpellId())
			 ...
			 case 48420:
				{
					if(GetUnitCaster()->HasAura(48412))
						md.value = 4;
					else if(GetUnitCaster()->HasAura(48411))
						md.value = 2;
				}break;
			}
...
void Aura::EventPeriodicManaLeech(uint32 amount)
{
	...
	uint32 amt = m_target->GetUInt32Value(UNIT_FIELD_MAXPOWER1);
	amt = amt*3/100;
	...
	if(amt >= mm)
		amt = mm;
	if(amt >= m_caster->GetUInt32Value(UNIT_FIELD_MAXPOWER1))
		amt = m_caster->GetUInt32Value(UNIT_FIELD_MAXPOWER1);

	//Log.Notice("spell", "amt = %u",m_caster->GetUInt32Value(UNIT_FIELD_POWER1) );
	m_caster->Energize(m_caster, GetSpellId(), (m_caster->GetUInt32Value(UNIT_FIELD_POWER1) + amt), POWER_TYPE_MANA);
	//Log.Notice("spell", "amt = %u",m_caster->GetUInt32Value(UNIT_FIELD_POWER1) );
	if(m_target->GetUInt32Value(UNIT_FIELD_POWER1) - amt <= 0)
		amt = 0;
...
void Aura::SpellAuraSchoolAbsorb(bool apply)
{
	if(apply)
	{
		...
		if( plr )
		{
			if(plr->HasAura(52800))
				plr->CastSpell(plr, 59891, true);
			else if(plr->HasAura(52799))
				plr->CastSpell(plr, 59890, true);
			else if(plr->HasAura(52798))
				plr->CastSpell(plr, 59889, true);
			else if(plr->HasAura(52797))
				plr->CastSpell(plr, 59888, true);
			else if(plr->HasAura(52795))
				plr->CastSpell(plr, 59887, true);
...
void Aura::SpellAuraMechanicImmunity(bool apply)
{
	if(apply)
	{
		...
		if(mod->m_miscValue != 16 && mod->m_miscValue != 25 && mod->m_miscValue != 19) // don't remove bandages, Power Word and protection effect
		{
			...
			if(m_spellProto->Id == 1044)
			{
				if(GetUnitCaster()->HasAura(31872))
				{
					for(uint32 x= MAX_NEGATIVE_AURAS_EXTEDED_START; x < MAX_NEGATIVE_AURAS_EXTEDED_END; ++x)
					{
						if(m_target->m_auras[x])
...
out1:
				...
				else if(GetUnitCaster()->HasAura(31871))
				{
			...
			if(m_spellProto->Id==42292)
			{
				...
				for(uint32 x= MAX_NEGATIVE_AURAS_EXTEDED_START; x < MAX_NEGATIVE_AURAS_EXTEDED_END; ++x)
				{
					if(m_target->m_auras[x])
					{
						for(uint32 y = 0; y < 3; ++y)
						{
							if(m_target->m_auras[x]->GetSpellId() == 51514)
								m_target->m_auras[x]->Remove();
							switch(m_target->m_auras[x]->GetSpellProto()->EffectApplyAuraName[y])
							{
							case SPELL_AURA_MOD_STUN:
...
void Aura::SpellAuraFeatherFall( bool apply )
{
	...
	TO_PLAYER(m_target)->m_fallDisabledUntil = UNIXTIME + GetDuration();
...
{
	if(GetSpellProto()->NameHash == SPELL_HASH_BATTLE_SHOUT )
	{
		SM_FIValue(static_cast< Player* >( m_target )->SM_FMiscEffect,&mod->m_amount,GetSpellProto()->SpellGroupType);
		SM_PIValue(static_cast< Player* >( m_target )->SM_PMiscEffect,&mod->m_amount,GetSpellProto()->SpellGroupType);
	}
	if(apply)
	{
		if (m_target->GetTypeId() == TYPEID_PLAYER && GetSpellProto()->NameHash == SPELL_HASH_ASPECT_OF_THE_DRAGONHAWK)
		{
			m_target->SetDodgeFromSpell(m_target->GetDodgeFromSpell() + 18 );
			static_cast< Player* >( m_target )->UpdateChances();
		}
		if(mod->m_amount > 0)
			SetPositive();
...
void Aura::SpellAuraIncreaseMaxHealth(bool apply)
{
	...
	SM_FIValue(static_cast< Player* >( m_target )->SM_FMiscEffect,&amount,GetSpellProto()->SpellGroupType);
	SM_PIValue(static_cast< Player* >( m_target )->SM_PMiscEffect,&amount,GetSpellProto()->SpellGroupType);
...
void Aura::SpellAuraAllowOnlyAbility(bool apply)
{
	...
	if ( !GetUnitCaster()->IsPlayer() && GetSpellId() != 48018 ) 
		return;

	// Generic
	if( apply )
	{
		static_cast<Player*>(GetUnitCaster())->m_castFilterEnabled = true;
		for( uint32 x=0; x<3; x++ )
			static_cast<Player*>(GetUnitCaster())->m_castFilter[x] |= m_spellProto->EffectSpellClassMask[mod->i][x];
		static_cast<Player*>(GetUnitCaster())->SetFlag(PLAYER_FLAGS, PLAYER_ALLOW_ONLY_ABILITY);
	}
	else
	{
		static_cast<Player*>(GetUnitCaster())->m_castFilterEnabled = false;	// check if we can turn it off
		for( uint32 x=0; x<3; x++ )
		{
			static_cast<Player*>(GetUnitCaster())->m_castFilter[x] &= ~m_spellProto->EffectSpellClassMask[mod->i][x];
			if(static_cast<Player*>(GetUnitCaster())->m_castFilter[x])
				static_cast<Player*>(GetUnitCaster())->m_castFilterEnabled = true;
		}
		static_cast<Player*>(GetUnitCaster())->RemoveFlag(PLAYER_FLAGS, PLAYER_ALLOW_ONLY_ABILITY);
	}
}
...
void Aura::SpellAuraConvertRuneInDeath(bool apply)
{
	if(!apply)
	{
		if(TO_PLAYER(GetUnitCaster())->m_runes[0] == RUNE_DEATH)
			TO_PLAYER(GetUnitCaster())->ConvertRune(0, 0);
		else if(TO_PLAYER(GetUnitCaster())->m_runes[1] == RUNE_DEATH)
			TO_PLAYER(GetUnitCaster())->ConvertRune(1, 0);
	}
}
void Aura::SpellAuraAuraCloneCaster( bool apply )
{
    if (apply)
    {
        Unit * caster = GetUnitCaster();
		...
		m_target->SetUInt32Value(UNIT_FIELD_NATIVEDISPLAYID, (caster->GetUInt32Value(UNIT_FIELD_NATIVEDISPLAYID)));
		m_target->SetUInt32Value(UNIT_FIELD_DISPLAYID, (caster->GetUInt32Value(UNIT_FIELD_DISPLAYID)));
		m_target->EventModelChange();
...
void Aura::SpellAuraCallStabledPet(bool apply)
{
	...
	uint32 PetNum = 0;
	for(std::map<uint32, PlayerPet*>::iterator itr = p_caster->m_Pets.begin(); itr != p_caster->m_Pets.end(); ++itr)
	{
		if(itr->second->stablestate == 2)
			PetNum = itr->first;
	}
	PlayerPet *pet = p_caster->GetPlayerPet(PetNum);

	PlayerPet *pPet = p_caster->GetPlayerPet( p_caster->GetUnstabledPetNumber() );
	Pet *pet2 = p_caster->GetSummon();
	...
	pet2->Remove( true, true );
	pPet->stablestate = STABLE_STATE_PASSIVE;

	if(!pet)
	{
		sLog.outError("PET SYSTEM: Player "I64FMT" tried to unstable non-existent pet %d", p_caster->GetGUID(), PetNum);
		return;
	}

	//unstable selected pet
	p_caster->SpawnPet( PetNum );
