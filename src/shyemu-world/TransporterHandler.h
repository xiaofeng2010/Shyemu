...
public:
	...
	SHYEMU_INLINE void SetLength(const unsigned int sz)
	{
		i_nodes.resize( sz );
	}

	SHYEMU_INLINE size_t Size(void) const { return i_nodes.size(); }
	SHYEMU_INLINE void Resize(unsigned int sz) { i_nodes.resize(sz); }
	SHYEMU_INLINE void Clear(void) { i_nodes.clear(); }
	SHYEMU_INLINE PathNode* GetNodes(void) { return static_cast<PathNode *>(&i_nodes[0]); }
...
public:
	...
	SHYEMU_INLINE void AddPlayer(Player *pPlayer) { mPassengers[pPlayer->GetLowGUID()] = pPlayer; }
	SHYEMU_INLINE void RemovePlayer(Player *pPlayer) {mPassengers.erase(pPlayer->GetLowGUID()); }
	SHYEMU_INLINE bool HasPlayer(Player* pPlayer) { return mPassengers.find(pPlayer->GetLowGUID()) != mPassengers.end(); }
	SHYEMU_INLINE void SetPeriod(uint32 val) { m_period = val; }
