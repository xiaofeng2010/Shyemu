...
enum PlayerFlags
{
	...
	PLAYER_FLAGS_UNK20				= 0x80000,
    PLAYER_FLAGS_UNK21				= 0x100000,
    PLAYER_FLAGS_UNK22				= 0x200000,
    PLAYER_FLAGS_UNK23				= 0x400000,
    PLAYER_ALLOW_ONLY_ABILITY		= 0x800000,                // used by bladestorm and killing spree
    PLAYER_FLAGS_UNK25				= 0x1000000                 // disabled all melee ability on tab include autoattack
};
...
struct FactionReputation
{
	...
	uint8 flag;
	int32 baseStanding;
	SHYEMU_INLINE int32 CalcStanding() { return standing - baseStanding; }
	SHYEMU_INLINE bool Positive() { return standing >= 0; }
};
...
public:
	...
	SHYEMU_INLINE const char* GetName() { return m_name.c_str(); }
	SHYEMU_INLINE std::string* GetNameString() { return &m_name; }
	void Die();
    ...
    /* Taxi                                                                 */
    /************************************************************************/
    SHYEMU_INLINE TaxiPath*    GetTaxiPath() { return m_CurrentTaxiPath; }
    SHYEMU_INLINE bool         GetTaxiState() { return m_onTaxi; }
    ...
    SHYEMU_INLINE void         SetTaxiState    (bool state) { m_onTaxi = state; }
    SHYEMU_INLINE void         SetTaximask     (uint8 index, uint32 value ) { m_taximask[index] = value; }
    SHYEMU_INLINE void         SetTaxiPath     (TaxiPath *path) { m_CurrentTaxiPath = path; }
    SHYEMU_INLINE void         SetTaxiPos()	{m_taxi_pos_x = m_position.x; m_taxi_pos_y = m_position.y; m_taxi_pos_z = m_position.z;}
    SHYEMU_INLINE void         UnSetTaxiPos()	{m_taxi_pos_x = 0; m_taxi_pos_y = 0; m_taxi_pos_z = 0; }
	...
	SHYEMU_INLINE QuestLogEntry*GetQuestLogInSlot(uint32 slot)  { return m_questlog[slot]; }
    SHYEMU_INLINE uint32        GetQuestSharer()                { return m_questSharer; }

    SHYEMU_INLINE void         SetQuestSharer(uint32 guid)     { m_questSharer = guid; }
    void                SetQuestLogSlot(QuestLogEntry *entry, uint32 slot);

    SHYEMU_INLINE void         PushToRemovedQuests(uint32 questid)	{ m_removequests.insert(questid);}
	SHYEMU_INLINE void			PushToFinishedDailies(uint32 questid) { DailyMutex.Acquire(); m_finishedDailies.insert(questid); DailyMutex.Release();}
	SHYEMU_INLINE bool		HasFinishedDaily(uint32 questid) { return (m_finishedDailies.find(questid) == m_finishedDailies.end() ? false : true); }
	...
	SHYEMU_INLINE uint32 GetTeam() { return m_team; }
	SHYEMU_INLINE uint32 GetTeamInitial() { return myRace->team_id==7 ? 0 : 1; }
	SHYEMU_INLINE void SetTeam(uint32 t) { m_team = t; m_bgTeam=t; }
	SHYEMU_INLINE void ResetTeam() { m_team = myRace->team_id==7 ? 0 : 1; m_bgTeam=m_team; }

	SHYEMU_INLINE bool IsInFeralForm()
	{
		int s = GetShapeShift();
		if( s <= 0 )
			return false;
	...
	SHYEMU_INLINE uint32       GetInviter() { return m_GroupInviter; }
	SHYEMU_INLINE bool         InGroup() { return (m_playerInfo->m_Group != NULL && !m_GroupInviter); }
	bool                IsGroupLeader()
	{
	...
	SHYEMU_INLINE int          HasBeenInvited() { return m_GroupInviter != 0; }
	SHYEMU_INLINE Group*       GetGroup() { return m_playerInfo ? m_playerInfo->m_Group : NULL; }
	SHYEMU_INLINE int8		   GetSubGroup() { return m_playerInfo->subGroup; }
    bool                IsGroupMember(Player *plyr);
	SHYEMU_INLINE bool         IsBanned()
	{
		if(m_banned)
		{
    ...
    SHYEMU_INLINE void         SetBanned() { m_banned = 4;}
	SHYEMU_INLINE void         SetBanned(string Reason) { m_banned = 4; m_banreason = Reason;}
	SHYEMU_INLINE void         SetBanned(uint32 timestamp, string& Reason) { m_banned = timestamp; m_banreason = Reason; }
	SHYEMU_INLINE void         UnSetBanned() { m_banned = 0; }
	SHYEMU_INLINE string       GetBanReason() {return m_banreason;}

    /************************************************************************/
    /* Guilds                                                               */
    /************************************************************************/
	SHYEMU_INLINE Guild *		GetGuild() { return m_playerInfo->guild; }
	SHYEMU_INLINE bool			IsInGuild() {return (m_uint32Values[PLAYER_GUILDID] != 0) ? true : false;}
	SHYEMU_INLINE uint32		GetGuildId() { return m_uint32Values[PLAYER_GUILDID]; }
	void						SetGuildId(uint32 guildId);
	SHYEMU_INLINE uint32		GetGuildRank() { return m_uint32Values[PLAYER_GUILDRANK]; }
	SHYEMU_INLINE GuildRank*	GetGuildRankS() { return m_playerInfo->guildRank; }
	...
	SHYEMU_INLINE GuildMember * GetGuildMember() { return m_playerInfo->guildMember; }
	...
	SHYEMU_INLINE uint8        GetDuelStatus() { return m_duelStatus; }
    ...
    /* Pets                                                                 */
    /************************************************************************/
	SHYEMU_INLINE void			SetSummon(Pet *pet) { m_Summon = pet; }
	SHYEMU_INLINE Pet*			GetSummon(void) { return m_Summon; }
	uint32						GeneratePetNumber(void);
	void						RemovePlayerPet(uint32 pet_number);
	SHYEMU_INLINE void			AddPlayerPet(PlayerPet* pet, uint32 index) { m_Pets[index] = pet; }
	SHYEMU_INLINE PlayerPet*	GetPlayerPet(uint32 idx)
	{
	...
	SHYEMU_INLINE uint8         GetPetCount(void) { return (uint8)m_Pets.size(); }
	SHYEMU_INLINE void			SetStableSlotCount(uint8 count) { m_StableSlotCount = count; }
	SHYEMU_INLINE uint8			GetStableSlotCount(void) { return m_StableSlotCount; }
    ...
    /* Item Interface                                                       */
    /************************************************************************/
	SHYEMU_INLINE ItemInterface* GetItemInterface() { return m_ItemInterface; } // Player Inventory Item storage
	SHYEMU_INLINE void			ApplyItemMods(Item *item, int16 slot, bool apply,bool justdrokedown=false) {  _ApplyItemMods(item, slot, apply,justdrokedown); }
	SHYEMU_INLINE bool			HasItemCount( uint32 item, uint32 count, bool inBankAlso = false ) const;
    ...
    /* Loot                                                                 */
    /************************************************************************/
	SHYEMU_INLINE const uint64& GetLootGUID() const { return m_lootGuid; }
	SHYEMU_INLINE void         SetLootGUID(const uint64 &guid) { m_lootGuid = guid; }
    ...
    /* World Session                                                        */
    /************************************************************************/
	SHYEMU_INLINE WorldSession* GetSession() const { return m_session; }
	...
	//SHYEMU_INLINE std::list<struct skilllines>getSkillLines() { return m_skilllines; }
	...
	SHYEMU_INLINE float GetSpellCritFromSpell() { return m_spellcritfromspell; }
	SHYEMU_INLINE float GetHitFromSpell() { return m_hitfromspell; }
	void SetSpellCritFromSpell(float value) { m_spellcritfromspell = value; }
	void SetHitFromSpell(float value) { m_hitfromspell = value; }
	SHYEMU_INLINE uint32 GetHealthFromSpell() { return m_healthfromspell; }
	SHYEMU_INLINE uint32 GetManaFromSpell() { return m_manafromspell; }
	...
	SHYEMU_INLINE void SetTalentResetTimes(uint32 value) { m_talentresettimes = value; }
	void SetPlayerStatus(uint8 pStatus) { m_status = pStatus; }
	SHYEMU_INLINE uint8 GetPlayerStatus() { return m_status; }
	...
	SHYEMU_INLINE uint8 GetShapeShift()
	{
	...
	SHYEMU_INLINE bool IsVisible(Object* pObj) { return !(m_visibleObjects.find(pObj) == m_visibleObjects.end()); }
	void AddInRangeObject(Object* pObj);
	void OnRemoveInRangeObject(Object* pObj);
	void ClearInRangeSet();
	SHYEMU_INLINE void AddVisibleObject(Object* pObj) { m_visibleObjects.insert(pObj); }
	SHYEMU_INLINE void RemoveVisibleObject(Object* pObj) { m_visibleObjects.erase(pObj); }
	SHYEMU_INLINE void RemoveVisibleObject(InRangeSet::iterator itr) { m_visibleObjects.erase(itr); }
	SHYEMU_INLINE InRangeSet::iterator FindVisible(Object * obj) { return m_visibleObjects.find(obj); }
	SHYEMU_INLINE void RemoveIfVisible(Object * obj)
	{
	...
	SHYEMU_INLINE bool GetVisibility(Object * obj, InRangeSet::iterator *itr)
	{
		*itr = m_visibleObjects.find(obj);
		return ((*itr) != m_visibleObjects.end());
	}

	SHYEMU_INLINE InRangeSet::iterator GetVisibleSetBegin() { return m_visibleObjects.begin(); }
	SHYEMU_INLINE InRangeSet::iterator GetVisibleSetEnd() { return m_visibleObjects.end(); }
	...
	SHYEMU_INLINE float res_M_crit_get(){return m_resist_critical[0];}
	SHYEMU_INLINE void res_M_crit_set(float newvalue){m_resist_critical[0]=newvalue;}
	SHYEMU_INLINE float res_R_crit_get(){return m_resist_critical[1];}
	SHYEMU_INLINE void res_R_crit_set(float newvalue){m_resist_critical[1]=newvalue;}
...
*/
	SHYEMU_INLINE uint32* GetPlayedtime() { return m_playedtime; };
    ...
    SHYEMU_INLINE uint32 GetSoulStoneReceiver(){return SoulStoneReceiver;}
    SHYEMU_INLINE void SetSoulStoneReceiver(uint32 StoneGUID){SoulStoneReceiver = StoneGUID;}
    SHYEMU_INLINE uint32 GetSoulStone(){return SoulStone;}
    SHYEMU_INLINE void SetSoulStone(uint32 StoneID){SoulStone = StoneID;}

	uint64 misdirectionTarget;

	SHYEMU_INLINE uint64 GetMisdirectionTarget(){return misdirectionTarget;}
	SHYEMU_INLINE void SetMisdirectionTarget(uint64 PlayerGUID){misdirectionTarget = PlayerGUID;}
	...
	SHYEMU_INLINE bool IsMounted() {return (m_MountSpellId!=0 ? true : false); }
	...
	SHYEMU_INLINE uint32 GetAreaID() { return m_AreaID; }
	void SetAreaID(uint32 area) { m_AreaID = area; }
	bool IsInCity();

	// Instance IDs

	SHYEMU_INLINE uint32 GetPersistentInstanceId(uint32 mapId, uint32 difficulty)
	{
...
public:
	...
	SHYEMU_INLINE void setMyCorpse(Corpse * corpse) { myCorpse = corpse; }
	SHYEMU_INLINE Corpse * getMyCorpse() { return myCorpse; }
...
//		RemoveByteFlag(UNIT_FIELD_BYTES_2, 1, 0x28);
	...
	SHYEMU_INLINE void RemovePvPFlag()
	{
		StopPvPTimer();
	...
	SHYEMU_INLINE bool IsFFAPvPFlagged()
	{
		return HasByteFlag(UNIT_FIELD_BYTES_2, 1, U_FIELD_BYTES_FLAG_FFA_PVP);
	}

	SHYEMU_INLINE void SetFFAPvPFlag()
	{
		StopPvPTimer();
	...
	SHYEMU_INLINE void RemoveFFAPvPFlag()
	{
		StopPvPTimer();
	...
	//! Do this on /pvp off
	SHYEMU_INLINE void ResetPvPTimer();
	//! Stop the timer for pvp off
	SHYEMU_INLINE void StopPvPTimer() { m_pvpTimer = 0; }
	...
	SHYEMU_INLINE uint32 LastHonorResetTime() const { return m_lastHonorResetTime; }
	SHYEMU_INLINE void LastHonorResetTime(uint32 val) { m_lastHonorResetTime = val; }
	uint32 OnlineTime;
	bool tutorialsDirty;
	LevelInfo * lvlinfo;
	...
	SHYEMU_INLINE void SetName(string& name) { m_name = name; }
	...
	SHYEMU_INLINE void AddComboPoints(uint64 target, uint8 count)
	{
	...
	SHYEMU_INLINE void NullComboPoints() { if(!m_retainComboPoints) { m_comboTarget = 0; m_comboPoints = 0; m_spellcomboPoints=0; } UpdateComboPoints(); }
	...
	SHYEMU_INLINE Player * GetTradeTarget()
	{
	...
	/* Last Speeds */
	SHYEMU_INLINE void UpdateLastSpeeds()
	{
	...
	SHYEMU_INLINE bool IsAttacking() {return m_attacking; }
...
public:
	...
	SHYEMU_INLINE PlayerInfo * getPlayerInfo() const { return m_playerInfo; }
	...
	SHYEMU_INLINE uint8 GetRune(uint32 index)
	{
		ASSERT(index < 6);
...
#ifdef ENABLE_ACHIEVEMENTS
    SHYEMU_INLINE AchievementMgr& GetAchievementMgr() { return m_achievementMgr; }
...
public:
	...
	SHYEMU_INLINE Vehicle * GetVehicle() { return (Vehicle *)m_vehicle; }
	SHYEMU_INLINE int8 GetVehicleSeat() { return m_vehicleSeat; }
	SHYEMU_INLINE void SetVehicle(Vehicle *v, int8 seat)
	{
		m_vehicle = (Unit *)v;
		m_vehicleSeat = seat;
	}
	SHYEMU_INLINE void ResetVehicleSettings()
	{
		m_vehicle = NULL;
		m_vehicleSeat = -1;
	}
	...
	SHYEMU_INLINE const uint8 *GetTalentInspectBuffer() { return m_talentInspectBuffer; }
...
public:
	...
	SHYEMU_INLINE PlayerSkill* Grab() { return &m_itr->second; }
	SHYEMU_INLINE bool End() { return (m_itr==m_endItr)?true:false; }
