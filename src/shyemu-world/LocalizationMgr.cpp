...
#define SAFE_FREE_PTR(x) if(deletedPointers.find((x)) == deletedPointers.end()) { deletedPointers.insert((x)); free((x)); }
    ...
    for(i = 0; i < maxid; ++i)
	{
		...
		for(HM_NAMESPACE::hash_map<uint32, LocalizedInstanceStringTable>::iterator itr = m_InstanceStrings[i].begin(); itr != m_InstanceStrings[i].end(); ++itr)
		{
			SAFE_FREE_PTR(itr->second.Text);
		}
...
void LocalizationMgr::Reload(bool first)
{
	...
	GetDistinctLanguages(languages, "instancestring_tables_localized");
	...
	m_InstanceStrings = new HM_NAMESPACE::hash_map<uint32, LocalizedInstanceStringTable>[langid];
	...
	/************************************************************************/
	/* Instance Common Message                                              */
	/************************************************************************/
	{
		LocalizedInstanceStringTable nt;
		string str;
		uint32 entry;
		Field * f;
		uint32 lid;

		result = WorldDatabase.Query("SELECT * FROM instancestring_tables_localized");
		if(result)
		{
			do 
			{
				...
				nt.Text = strdup(f[2].GetString());
				m_InstanceStrings[lid].insert(make_pair(entry, nt));
			} while(result->NextRow());
			delete result;
		}
	}
	...
	for(map<string,string>::iterator itr = bound_languages.begin(); itr != bound_languages.end(); ++itr)
	{
		...
		CopyHashMap<LocalizedInstanceStringTable>(&m_InstanceStrings[source_language_id], &m_InstanceStrings[dest_language_id]);
...
MAKE_LOOKUP_FUNCTION(LocalizedInstanceStringTable, m_InstanceStrings, GetLocalizedInstanceStringTable);
