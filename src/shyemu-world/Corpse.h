...
public:
   ...
   // void Create();
	...
	void SaveToDB();
	void DeleteFromDB();
	SHYEMU_INLINE void SetCorpseState(uint32 state) { m_state = state; }
	SHYEMU_INLINE uint32 GetCorpseState() { return m_state; }
	void Despawn();
	
	SHYEMU_INLINE void SetLoadedFromDB(bool value) { _loadedfromdb = value; }
	SHYEMU_INLINE bool GetLoadedFromDB(void) { return _loadedfromdb; }
	Loot loot;
	void generateLoot();
