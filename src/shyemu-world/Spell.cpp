...
uint8 Spell::prepare( SpellCastTargets * targets )
{
	...
	if(i_caster && p_caster && i_caster->IsPotion() )
		p_caster->m_lastPotionId = i_caster->GetEntry();
	...
	if( p_caster && m_spellInfo->Id == 48018)
		p_caster->RemoveAura(48018);
...
void Spell::cast(bool check)
{
	...
	if(cancastresult == SPELL_CANCAST_OK)
	{
		...
		if( !(hasAttribute(ATTRIBUTE_ON_NEXT_ATTACK) && !m_triggeredSpell) )//on next attack
		{
			...
			{
				if( GetProto()->ChannelInterruptFlags != 0 && !m_triggeredSpell && m_spellInfo->NameHash != SPELL_HASH_MEND_PET)
				{
					/*
					...
					*/
					Log.Notice("ChannelInterruptFlags", "ChannelInterruptFlags = %u", GetProto()->ChannelInterruptFlags);
...
bool Spell::TakePower()
{
	...
	else
	{
		if(cost <= currentPower) // Unit has enough power (needed for creatures)
		{
			if( m_caster->IsPlayer() && static_cast<Unit*>(m_caster)->getClass() == WARRIOR )
				u_caster->m_LastRagePower = currentPower/10;
...
{   
	...
	{	
		//Log.Notice("yo" ," %u", m_spellInfo->Effect[i]);
...
uint8 Spell::CanCast(bool tolerate)
{
	...
	if(m_spellInfo->Id == 781 && !u_caster->CombatStatus.IsInCombat())
	...
	if( p_caster )
	{
		...
		*	Arena spell check, is cooldown longer then 15 minutes?
		 ...
		 *	Aura check
		 */
		if (GetProto()->casterAuraSpell && GetProto()->Id != 48020 && !p_caster->HasAura( GetProto()->casterAuraSpell ))
		{
			return SPELL_FAILED_NOT_READY;
		}
		if (GetProto()->casterAuraSpellNot && GetProto()->Id != 48020 &&  p_caster->HasAura( GetProto()->casterAuraSpellNot ))
		{
			return SPELL_FAILED_NOT_READY;
		}
		...
		if (target)
		{
			...
			switch(GetProto()->Id)
			{
				case 1515: // tame beast
				{
					...
					else if( !p_caster->HasAura(53270) && tame->IsExotic() )
...
exit:
	...
	else if( GetProto()->NameHash == SPELL_HASH_REND) // Rend
	{
		if(p_caster != NULL)
		{
			...
			case 772:
				{
					p_caster->Strike(unitTarget, MELEE, GetProto(), 25, 0, 0, false, false);
				}break;
			case 6546:
				{
					p_caster->Strike(unitTarget, MELEE, GetProto(), 40, 0, 0, false, false);
				}break;
			case 6547:
				{
					p_caster->Strike(unitTarget, MELEE, GetProto(), 50, 0, 0, false, false);
				}break;
			case 6548:
				{
					p_caster->Strike(unitTarget, MELEE, GetProto(), 70, 0, 0, false, false);
				}break;
			case 11572:
				{
					p_caster->Strike(unitTarget, MELEE, GetProto(), 115, 0, 0, false, false);
				}break;
			case 11573:
				{
					p_caster->Strike(unitTarget, MELEE, GetProto(), 150, 0, 0, false, false);
				}break;
			case 11574:
				{
					p_caster->Strike(unitTarget, MELEE, GetProto(), 185, 0, 0, false, false);
				}break;
			case 25208:
				{
					p_caster->Strike(unitTarget, MELEE, GetProto(), 215, 0, 0, false, false);
				}break;
			case 46845:
				{
					p_caster->Strike(unitTarget, MELEE, GetProto(), 315, 0, 0, false, false);
				}break;
			case 47465:
				{
					p_caster->Strike(unitTarget, MELEE, GetProto(), 380, 0, 0, false, false);
				}break;
			}
			Item *it;
			if(p_caster->GetItemInterface())
...
void Spell::Heal(int32 amount, bool ForceCrit)
{
	...
	if( u_caster != NULL )
	{
		...
		if( ((critical = Rand(critchance))  != 0) || ForceCrit )
		{
			...
			Log.Notice("test","putain de merde");
			unitTarget->m_LastHealAmount = amount;
