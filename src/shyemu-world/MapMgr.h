...
#define IS_RESETABLE_INSTANCE(p) (( !(p)->m_persistent && ((p)->m_mapInfo->type == INSTANCE_NONRAID || ((p)->m_mapInfo->type == INSTANCE_ARENA && (p)->m_difficulty == MODE_NORMAL)) ) || (p)->m_persistent && ((p)->m_mapInfo->type == INSTANCE_RAID))
...
/////////////////////////////////////////////
	...
	SHYEMU_INLINE uint32 GenerateGameobjectGuid() { return ++m_GOHighGuid; }
	SHYEMU_INLINE GameObject * GetGameObject(uint32 guid)
	{
...
////////////////////////////////////////////
	...
	SHYEMU_INLINE DynamicObject * GetDynamicObject(uint32 guid)
	{
...
///////////////////////////////////
	...
	// Terrain Functions
	SHYEMU_INLINE float  GetLandHeight(float x, float y) { return GetBaseMap()->GetLandHeight(x, y); }
	SHYEMU_INLINE bool   IsUnderground(float x, float y,float z) { return GetBaseMap()->GetLandHeight(x, y) > (z+0.5f); }
	SHYEMU_INLINE float  GetWaterHeight(float x, float y) { return GetBaseMap()->GetWaterHeight(x, y); }
	SHYEMU_INLINE uint8  GetWaterType(float x, float y) { return GetBaseMap()->GetWaterType(x, y); }
	SHYEMU_INLINE uint8  GetWalkableState(float x, float y) { return GetBaseMap()->GetWalkableState(x, y); }
	SHYEMU_INLINE uint16 GetAreaID(float x, float y) { return GetBaseMap()->GetAreaID(x, y); }

	SHYEMU_INLINE uint32 GetMapId() { return _mapId; }

	void PushToProcessed(Player* plr);

	SHYEMU_INLINE bool HasPlayers() { return (m_PlayerStorage.size() > 0); }
	SHYEMU_INLINE bool IsCombatInProgress() { return (_combatProgress.size() > 0); }
	void TeleportPlayers();

	SHYEMU_INLINE uint32 GetInstanceID() { return m_instanceID; }
	SHYEMU_INLINE MapInfo *GetMapInfo() { return pMapInfo; }

	bool _shutdown;

	SHYEMU_INLINE MapScriptInterface * GetInterface() { return ScriptInterface; }
	virtual int32 event_GetInstanceID() { return m_instanceID; }

	void LoadAllCells();
	SHYEMU_INLINE size_t GetPlayerCount() { return m_PlayerStorage.size(); }
	...
	//SHYEMU_INLINE uint32 GetWorldState(uint32 state);
