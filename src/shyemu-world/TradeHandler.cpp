...
void WorldSession::HandleCancelTrade(WorldPacket & recv_data)
{
	...
	//Sauvegarde du TradeStatus
	uint32 TradeStatus = TRADE_STATUS_CANCELLED;

	if(_player->mTradeStatus == TRADE_STATUS_COMPLETE)
		TradeStatus = TRADE_STATUS_COMPLETE;
	else if( pTarget == NULL || !pTarget->IsInWorld())
		TradeStatus = TRADE_STATUS_PLAYER_NOT_FOUND;
	//Fin de la sauvegarde du TradeStatus

	//Reset des variables de Trades
	if(pTarget && pTarget->IsInWorld())
		pTarget->ResetTradeVariables();
	_player->ResetTradeVariables();
	//Fin des resets de variables

	//Envoie du Status
	if(pTarget && pTarget->IsInWorld() && pTarget->m_session && pTarget->m_session->GetSocket())
		pTarget->m_session->OutPacket(SMSG_TRADE_STATUS, 4, &TradeStatus);
	
	OutPacket(SMSG_TRADE_STATUS, 4, &TradeStatus);
	//Fin de l'envoie du status
}
