...
SHYEMU_INLINE bool FindXinYString(std::string& x, std::string& y)
{
...
public:
	...
	SHYEMU_INLINE void SetTextID(uint32 TID) { TextId = TID; }
...
public:
	...
	SHYEMU_INLINE uint32 GetLeader() { return LeaderGuid; }
	SHYEMU_INLINE uint32 GetID() { return CharterId; }

	SHYEMU_INLINE bool IsFull() { return (SignatureCount == Slots); }
};
...
#ifndef WIN32
#define shyemu_USE_MAP_PLAYER_INDEX
#ifdef shyemu_USE_MAP_PLAYER_INDEX
...
public:
	...
	SHYEMU_INLINE bool IsSpellDisabled(uint32 spellid)
	{
	...
	SHYEMU_INLINE GuildMap::iterator GetGuildsBegin() { return mGuild.begin(); }
	SHYEMU_INLINE GuildMap::iterator GetGuildsEnd() { return mGuild.end(); }
	...
	SHYEMU_INLINE uint32 GetBCGroupCountByKey(uint32 Key) { return (uint32)m_BCEntryStorage.count(Key); }
	SHYEMU_INLINE bool IsBCEntryStorageEmpty() { return m_BCEntryStorage.empty(); }
	SHYEMU_INLINE BCEntryStorage::iterator GetBCTotalItemBegin() { return m_BCEntryStorage.begin(); }
	SHYEMU_INLINE BCEntryStorage::iterator GetBCTotalItemEnd() { return m_BCEntryStorage.end(); }
	SHYEMU_INLINE int CalcCurrentBCEntry() 
