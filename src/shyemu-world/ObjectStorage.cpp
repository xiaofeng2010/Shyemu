...
const char * gCreatureProtoFormat						= "uuuuuuufuuuffuuffuuuuuuuffsuuuufffuuuuuuuu";
...
const char * gInstanceStringTableFormat					= "us";
...
SERVER_DECL SQLStorage<InstanceStringTable, HashMapStorageContainer<InstanceStringTable> >		InstanceStringTableStorage;
...
void ObjectMgr::LoadExtraCreatureProtoStuff()
{
	...
	QueryResult * result1 = WorldDatabase.Query( "SELECT entry, canFlee, canCallHelp, isAllowedToEnterCombat FROM creature_ai" );
	CreatureProto * cn1 = NULL;

	if( result1 != NULL )
	{
		uint32 entry = 0;
		bool canFlee = false;
		bool canCallHelp = false;
		bool AllToentercombat = true;
		
		do
		{
			Field *fields = result1->Fetch();
			entry = fields[0].GetUInt32();
			canFlee = fields[1].GetBool();
			canCallHelp = fields[2].GetBool();
			AllToentercombat = fields[3].GetBool();

			cn1 = CreatureProtoStorage.LookupEntry(entry);
			if(  cn1 == NULL )
			{
				Log.Warning("Creature_AI", "Agent skipped, NPC %u does not exist.", fields[0].GetUInt32());
				continue;
			}
			cn1->m_canFlee = canFlee;
			cn1->m_canCallForHelp = canCallHelp;
			cn1->m_isAllToEnterCombat = AllToentercombat;
...
void Storage_FillTaskList(TaskList & tl)
{
	...
	make_task(InstanceStringTableStorage, InstanceStringTable, HashMapStorageContainer, "instancestring_tables", gInstanceStringTableFormat);
}
...
void Storage_Cleanup()
{
	...
	InstanceStringTableStorage.Cleanup();
...
bool LoadAdditionalTable(const char * TableName, const char * SecondName)
{
	...
	else if(!stricmp(TableName, "instancestring_tables"))			// InstanceString
		InstanceStringTableStorage.LoadAdditionalData(SecondName, gInstanceStringTableFormat);
...
bool Storage_ReloadTable(const char * TableName)
{
	...
	else if(!stricmp(TableName, "instancestring_tables"))			// ist
		InstanceStringTableStorage.Reload();
