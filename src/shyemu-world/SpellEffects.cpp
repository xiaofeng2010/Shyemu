...
void Spell::SpellEffectDummy(uint32 i) // Dummy(Scripted events)
{
	...
	case 47471:
	{
		...
		int32 rageLeft = 0; // Nous utilisons tous la rage disponibles par défaut
		...
		{		
			...
			Mort_soudaine->Remove(); // Suppresion de l'aura aprs la 1ère execution
		...
		if (p_caster && skill)
		{
			...
			if ( discovered_recipe != 0 )
			{
				...
				if ( se )
				{
					...
					WorldPacket * data;
					char msg[256];
					sprintf( msg, "%sDecouverte! %s a découvert comment créer %s.|r", MSG_COLOR_GOLD, p_caster->GetName(), se->Name );
...
void Spell::SpellEffectWeaponDmgPerc(uint32 i) // Weapon Percent damage
{
	...
	// Chevalier de la mort Spell Codé dans SpellEffectDummyMelee
...
void Spell::SpellEffectDummyMelee( uint32 i ) // Normalized Weapon damage +
{
	...
	switch( m_spellInfo->Id )	// Frappe du Fléau
	{
		// Rang 1
		case 55090:
			damage = 0 + 151.875;
	...
	switch( m_spellInfo->Id )	// Anéantissement
	{
		// Rang 1
		case 49020:
			damage = 0 + 198.4;
