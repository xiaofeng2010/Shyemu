...
#undef END_IN_RANGE_LOOP*/
	...
	if(obj->HasInRangeObjects())
	{
			...
			iter != obj->GetInRangeSetEnd();)
		{
			...
			else if( curObj->GetTypeId() == TYPEID_GAMEOBJECT && static_cast<GameObject*>(curObj)->GetInfo() && static_cast<GameObject*>(curObj)->GetInfo()->Type == 11)
				fRange = 0.0f;
			...
			//If the object announcing its position is a transport, or other special object, then deleting it from visible objects should be avoided. - By: VLack
				...
				fRange = 0.0f;
			else if( obj->GetTypeId() == TYPEID_UNIT && obj->IsCreature() && static_cast<Creature*>(obj)->GetCreatureInfo() && static_cast<Creature*>(obj)->GetCreatureInfo()->Rank == ELITE_WORLDBOSS)
				fRange = 0.0f;
