		...
		case GAMEOBJECT_TYPE_RITUAL:
		{
			...
			if ( i == goinfo->SpellFocus - 1 )
			{
				...
				else if ( goinfo->ID == 177193 ) // doom portal
				{
					...
					info = dbcSpell.LookupEntry(goinfo->sound2);
					if(!info)
						break;
					spell = SpellPool.PooledNew();
					if (!spell)
					...
					// summons demon
					info = dbcSpell.LookupEntry(goinfo->sound3);
					spell = SpellPool.PooledNew();
					if (!spell)
						return;
...
void WorldSession::HandleGameobjReportUseOpCode( WorldPacket& recv_data )   // CMSG_GAMEOBJ_REPORT_USE
{
    ...
    case 191590:
		{
			if(_player->HasQuest(12848))
			{
				float SSX = _player->GetPositionX();
				float SSY = _player->GetPositionY();
				float SSZ = _player->GetPositionZ();
				//float SSO = _player->GetOrientation();
				Log.Notice("fuck", "off");
				Creature* pCreature = _player->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(SSX, SSY, SSZ);
				if(!pCreature || !pCreature->isAlive())
					return;
				Log.Notice("fuck", "off");
				if (pCreature->GetEntry() == 29519 || pCreature->GetEntry() == 29565 || pCreature->GetEntry() == 29567 || pCreature->GetEntry() == 29520 || pCreature->GetEntry() == 29566)
				{
					Log.Notice("fuck", "off");
					_player->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, "Je te donne la clé pour te libérer.");
				    pCreature->SetUInt64Value(UNIT_FIELD_FLAGS, 0);
				    pCreature->GetAIInterface()->SetNextTarget(_player);
					pCreature->GetAIInterface()->AttackReaction(_player, 1, 0);
				}
			}
		}break;
	}
