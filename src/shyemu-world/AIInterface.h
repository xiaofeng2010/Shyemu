...
public:
	...
	SHYEMU_INLINE Unit* getUnitToFollow() { return UnitToFollow; }
	void setCreatureState(CreatureState state){ m_creatureState = state; }
	SHYEMU_INLINE uint8 getAIState() { return static_cast<uint8>( m_AIState ); }
	SHYEMU_INLINE uint8 getAIType() { return static_cast<uint8>( m_AIType ); }
	SHYEMU_INLINE uint8 getCurrentAgent() { return static_cast<uint8>( m_aiCurrentAgent ); }
	...
	SHYEMU_INLINE AssistTargetSet GetAssistTargets() { return m_assistTargets; }
	SHYEMU_INLINE void LockAITargets(bool lock) 
	{ 
		lock? m_aiTargetsLock.Acquire(): m_aiTargetsLock.Release(); 
	};
	SHYEMU_INLINE TargetMap *GetAITargets() { return &m_aiTargets; }
	...
	SHYEMU_INLINE size_t getAITargetsCount() { return m_aiTargets.size(); }
	SHYEMU_INLINE uint32 getOutOfCombatRange() { return m_outOfCombatRange; }
...
//	void CancelSpellCast();
	...
	SHYEMU_INLINE bool hasWaypoints() { return m_waypoints!=NULL; }
	SHYEMU_INLINE void setMoveType(uint32 movetype) { m_moveType = movetype; }
	SHYEMU_INLINE uint32 getMoveType() { return m_moveType; }
	SHYEMU_INLINE void setMoveRunFlag(bool f) { m_moveRun = f; }
	SHYEMU_INLINE bool getMoveRunFlag() { return m_moveRun; }
	...
	SHYEMU_INLINE bool GetAllowedToEnterCombat(void) { return m_AllowedToEnterCombat; }

	void CheckTarget(Unit* target);
	SHYEMU_INLINE void SetAIState(AI_State newstate) { m_AIState = newstate; }
	...
	SHYEMU_INLINE void AddStopTime(uint32 Time) { m_moveTimer += Time; }
	SHYEMU_INLINE void SetNextSpell(AI_Spell * sp) { m_nextSpell = sp; }
	Unit* GetNextTarget();
	void SetNextTarget (Unit *nextTarget);
	void SetNextTarget (uint64 nextTarget); 

	/*SHYEMU_INLINE void ResetProcCounts()
	{
		AI_Spell * sp;
	...
	SHYEMU_INLINE void SetPetOwner(Unit * owner) { m_PetOwner = owner; }
	...
	SHYEMU_INLINE void SetWaypointMap(WayPointMap * m) { m_waypoints = m; }
	bool m_isGuard;
