...
SHYEMU_INLINE float GetRadius(SpellRadius *radius)
{
	return radius->Radius;
}
SHYEMU_INLINE uint32 GetCastTime(SpellCastTime *time)
{
	return time->CastTime;
}
SHYEMU_INLINE float GetMaxRange(SpellRange *range)
{
	return range->maxRange;
}
SHYEMU_INLINE float GetMinRange(SpellRange *range)
{
	return range->minRange;
}
SHYEMU_INLINE uint32 GetDuration(SpellDuration *dur)
{
...
public:
	...
	SHYEMU_INLINE uint32 GetNumRows()
	{
		return m_numrows;
	}
