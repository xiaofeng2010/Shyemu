...
protected:
	HANDLE create_anonymous_semaphore(long cur,long max)
	{
		...
		SHYEMU_INLINE void shift_generations_down()
		{
		...
		SHYEMU_INLINE void dispose_entry(list_entry& entry)
		{
		...
		SHYEMU_INLINE HANDLE duplicate_handle(HANDLE source)
		{
...
{
public:
	SHYEMU_INLINE Condition(Mutex * mutex) : m_nLockCount(0), m_externalMutex(mutex)
	{
	...
	SHYEMU_INLINE void BeginSynchronized()
	{
		m_externalMutex->Acquire();
		++m_nLockCount;
	}

	SHYEMU_INLINE void EndSynchronized()
	{
...
{
public:
	SHYEMU_INLINE Condition(Mutex *m)
	{
		mut=m;
		pthread_cond_init(&cond,NULL);
	}
	SHYEMU_INLINE ~Condition()
	{
		pthread_cond_destroy(&cond);
	}

	SHYEMU_INLINE void Signal()
	{
		pthread_cond_signal(&cond);
	}
	SHYEMU_INLINE void Broadcast()
	{
		pthread_cond_broadcast(&cond);
	}
	SHYEMU_INLINE void Wait()
	{
		pthread_cond_wait(&cond,&mut->mutex);
	}
	SHYEMU_INLINE bool Wait(time_t seconds)
	{
		timespec tv;
		tv.tv_nsec = 0;
		tv.tv_sec = seconds;
