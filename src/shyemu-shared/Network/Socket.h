...
/* Sending Operations */
	...
	SHYEMU_INLINE void BurstBegin() { m_writeMutex.Acquire(); }
	...
	SHYEMU_INLINE void BurstEnd() { m_writeMutex.Release(); }

/* Client Operations */

	// Get the client's ip in numerical form.
	string GetRemoteIP();
	SHYEMU_INLINE uint32 GetRemotePort() { return ntohs(m_client.sin_port); }
	SHYEMU_INLINE SOCKET GetFd() { return m_fd; }
...
/* Platform-specific methods */
	...
	SHYEMU_INLINE bool IsDeleted() { return m_deleted; }
	SHYEMU_INLINE bool IsConnected() { return m_connected; }
	SHYEMU_INLINE sockaddr_in & GetRemoteStruct() { return m_client; }
	SHYEMU_INLINE CircularBuffer& GetReadBuffer() { return readBuffer; }
	SHYEMU_INLINE CircularBuffer& GetWriteBuffer() { return writeBuffer; }

/* Deletion */
	void Delete();

	SHYEMU_INLINE in_addr GetRemoteAddress() { return m_client.sin_addr; }
...
public:
	...
	SHYEMU_INLINE void SetCompletionPort(HANDLE cp) { m_completionPort = cp; }
	
	// Atomic wrapper functions for increasing read/write locks
	SHYEMU_INLINE void IncSendLock() { InterlockedIncrement(&m_writeLock); }
	SHYEMU_INLINE void DecSendLock() { InterlockedDecrement(&m_writeLock); }
	SHYEMU_INLINE bool AcquireSendLock()
	{
		if(m_writeLock)
			return false;
		else
...
public:
	...
	// Atomic wrapper functions for increasing read/write locks
	SHYEMU_INLINE void IncSendLock() { m_writeLockMutex.Acquire(); m_writeLock++; m_writeLockMutex.Release(); }
	SHYEMU_INLINE void DecSendLock() { m_writeLockMutex.Acquire(); m_writeLock--; m_writeLockMutex.Release(); }
	SHYEMU_INLINE bool HasSendLock() { bool res; m_writeLockMutex.Acquire(); res = (m_writeLock != 0); m_writeLockMutex.Release(); return res; }
